import { DEFAULT_SITE_THEME, themeVariables, type SiteTheme } from './library/site-theme'
...
const THEME_TOKENS: Array<[RegExp, string]> = [
...
export function themed(markup: string) {
...
function makeShell(definition: FroamComponentDefinition, theme: SiteTheme) {
  ...
  for (const [name, value] of Object.entries(themeVariables(theme))) element.style.setProperty(name, value)
  ...
  if (isNav) element.style.borderBottom = '1px solid var(--fx-line)'
  if (isFooter) element.style.borderTop = '1px solid var(--fx-line)'
  return element
}

function action(label: string, primary = true) {
 ...
 */
function finishPattern(root: HTMLElement, definition: FroamComponentDefinition, theme: SiteTheme) {
  ...
  if (definition.category === 'Navigation' || definition.category === 'Footer') {
    root.querySelectorAll('nav a').forEach((link, index) => {
      if (theme.navLinks[index]) link.textContent = theme.navLinks[index]
    })
  }
  root.querySelectorAll<HTMLElement>('h1, h2, h3, blockquote').forEach((heading) => {
    heading.style.fontFamily = 'var(--fx-font-heading)'
  })
}

export function createFroamLibraryComponent(componentId: string, theme: SiteTheme = DEFAULT_SITE_THEME): HTMLElement | null {
