...
import { localScreenshotProvider, type FroamScreenshotRegion } from '../project/screenshot-reconstruction'
...
export default function FroamIntelligence(props: Props) {
  ...
  async function importScreenshot(file: File) {
    ...
    try {
      ...
      commit(events); props.onInsertReconstruction(result.regions, bitmap.width, bitmap.height); props.onToast(`Reconstructed ${result.regions.length} editable regions locally`)
  ...
  return <>
    <aside className="froam-intelligence" data-chef-editor-root="true">
      <header><div><strong>Froam Intelligence</strong><small>v7 · understands locally</small></div><button type="button" onClick={() => { props.onPreviewWidth(null); props.onClose() }}><X size={15} /></button></header>
      ...
      <main>
        ...
        {tab === 'archaeology' && <section><h3>Design Archaeology</h3>{archaeology ? <><dl><dt>Creation</dt><dd>{archaeology.creation ? `${archaeology.creation.actorId} · ${archaeology.creation.branchId}` : 'Unknown — no recorded creation event'}</dd><dt>Branch lineage</dt><dd>{archaeology.branchLineage.join(' → ')}</dd><dt>Authors</dt><dd>{archaeology.authors.join(', ') || 'Unknown'}</dd><dt>Derived from</dt><dd>{archaeology.derivedFrom.join(', ') || 'No recorded origin'}</dd></dl><div className="froam-intelligence__list">{archaeology.edits.slice().reverse().map((edit) => <article key={edit.eventId}><div><strong>{edit.label}</strong><small>{edit.actorId} · {edit.branchId}</small>{edit.rationale && <em>Recorded reason: {edit.rationale.text}</em>}</div></article>)}</div></> : <p>Select a node to trace recorded origins. Froam never invents rationale.</p>}</section>}
        ...
        {tab === 'screenshot' && <section><h3>Screenshot → Live UI</h3><p>Experimental local segmentation reconstructs editable structure. It does not recover original source code.</p><button type="button" className="is-primary" onClick={() => screenshotInput.current?.click()} disabled={screenshotBusy}>{screenshotBusy ? 'Reconstructing…' : 'Import screenshot'}</button><input ref={screenshotInput} hidden type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => { const file = event.target.files?.[0]; if (file) void importScreenshot(file); event.target.value = '' }} /><dl><dt>Provider</dt><dd>froam-local-segmentation-v1</dd><dt>Data boundary</dt><dd>Pixels stay in this browser. No source, credentials or image data is uploaded.</dd><dt>Multi-reference readiness</dt><dd>Each reconstruction is stored as a versioned analysis, ready for future desktop/mobile or state pairing.</dd></dl></section>}
      </main>
    </aside>
