...
type Seed = readonly [id: string, label: string, intent: string, keywords?: string]

const make = (category: FroamQuickEditCategory, seeds: readonly Seed[]): FroamQuickEditAction[] =>
  seeds.map(([id, label, intent, keywords = '']) => ({ id: `quick-edit:${id}`, label, intent, category, keywords }))
...
const values = (
  ...
  unit = '',
  keywords = '',
): FroamQuickEditAction[] =>
  make(category, list.map((value): Seed => [
...
const layout = make('Layout', [
  ...
  ...(['start', 'center', 'end', 'space-between', 'space-around', 'space-evenly'] as const).map((value): Seed =>
    [`justify-${value}`, `Justify content ${value.replace('space-', '')}`, `Set justify-content to ${value}`, 'flex alignment spread']),
  ...(['start', 'center', 'end', 'stretch', 'baseline'] as const).map((value): Seed =>
    [`align-${value}`, `Align items ${value}`, `Set align-items to ${value}`, 'flex alignment']),
...
const spacing = make('Spacing', [
  ...[0, 4, 8, 12, 16, 24, 32, 48].map((value): Seed => [`padding-${value}`, `Padding ${value}px`, `Set padding to ${value}px`, 'inset space']),
  ...[0, 8, 16, 24].map((value): Seed => [`margin-${value}`, `Margin ${value}px`, `Set margin to ${value}px`, 'outer space']),
  ...[8, 16, 24].map((value): Seed => [`gap-${value}`, `Gap ${value}px`, `Set gap to ${value}px`, 'item spacing']),
])

const surface = make('Surface', [
  ...[0, 4, 8, 12, 16, 24].map((value): Seed => [`radius-${value}`, `Corner radius ${value}px`, `Set corner radius to ${value}px`, 'rounded surface']),
...
const motion = make('Motion', [
  ...(['up', 'down', 'left', 'right'] as const).map((direction): Seed =>
    [`move-${direction}-8`, `Move ${direction} 8px`, `Move this ${direction} by 8px`, 'shift position']),
...
export function searchFroamQuickEdits(query: string, limit = 24): FroamQuickEditAction[] {
  ...
  return FROAM_QUICK_EDIT_ACTIONS
    ...
    .filter((match): match is { action: FroamQuickEditAction; score: number } => match !== null)
