...
function effectiveBackground(start: Element | null): Rgba {
 ...
 */
function sampleAccent(scope: ParentNode): Rgba | null {
  const votes = new Map<string, { color: Rgba; weight: number }>()
  const vote = (color: Rgba | null, weight: number) => {
    if (!color || color.a < 0.6 || chroma(color) < 0.3) return
    const lum = luminance(color)
    if (lum > 0.75 || lum < 0.012) return
    const key = toHex(color)
    const entry = votes.get(key) ?? { color, weight: 0 }
    entry.weight += weight
    votes.set(key, entry)
  }
  const controls = Array.from(scope.querySelectorAll<HTMLElement>('button, a, [role="button"], input[type="submit"]')).filter(isVisible).slice(0, 200)
  ...
  for (const accent of Array.from(scope.querySelectorAll<HTMLElement>('h1 *, h2 *, [class*="accent"], [class*="brand"], [class*="primary"], [class*="gradient"]')).slice(0, 80)) {
    const style = getComputedStyle(accent)
    vote(parseColor(style.color), 1.5)
    for (const stop of style.backgroundImage.match(/rgba?\([^)]*\)/g) ?? []) vote(parseColor(stop), 2)
  }
  // …and in the logo and icons: the SVG fills and strokes in the header and controls.
  for (const shape of Array.from(scope.querySelectorAll<SVGElement>('header svg *, nav svg *, a svg *, button svg *, [class*="logo" i] svg *')).slice(0, 80)) {
    const style = getComputedStyle(shape)
    vote(parseColor(style.fill), 2.5)
    vote(parseColor(style.stroke), 1.5)
  }
  let best: { color: Rgba; weight: number } | null = null
  for (const entry of votes.values()) if (!best || entry.weight > best.weight) best = entry
  return best?.color ?? null
}

function sampleRadius(elements: HTMLElement[], fallback: string) {
...
function sampleMaxWidth(scope: ParentNode) {
  const widths = Array.from(scope.querySelectorAll<HTMLElement>('main, section, header, footer, [class*="container"], [class*="wrapper"], main > *, section > *'))
    .slice(0, 120)
...
function sampleBrandName(scope: ParentNode) {
  const candidates = [
...
function sampleNavLinks(scope: ParentNode) {
...
export function sampleSiteTheme(scope: HTMLElement = document.body): SiteTheme {
  try {
    const bodyStyle = getComputedStyle(document.body)
    const heading = Array.from(scope.querySelectorAll<HTMLElement>('h1, h2')).find(isVisible)
    const paragraph = Array.from(scope.querySelectorAll<HTMLElement>('p')).find(isVisible)
    ...
    const cards = Array.from(scope.querySelectorAll<HTMLElement>('article, li, [class*="card"], img, figure')).slice(0, 80)
    const buttons = Array.from(scope.querySelectorAll<HTMLElement>('button, [class*="btn"], [role="button"]')).slice(0, 40)
    return {
