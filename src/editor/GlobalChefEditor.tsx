...
import { LOCAL_ACTOR, scopeKey, type FroamAnchor } from '../collab/types'
...
import { useCanvasPointer } from './chef/useCanvasPointer'
...
import {
  ...
  persistedStyleKeys,
} from './chef/style-options'
import {
  MAX_PERSONA_IMAGE_BYTES,
  SAVE_META_KEY,
  BRAND_FONT_MAX_BYTES,
  loadBrandFonts,
  saveBrandFontsForProject,
  loadStore,
  saveStoreForProject,
  loadNodeRegistry,
  saveNodeRegistryForProject,
  loadPersonaPreference,
  savePersonaPreference,
  personasEqual,
  stripPersonaDrafts,
  withPersonaDraft,
  countRenderableDrafts,
} from './chef/storage'
import {
  getRoot,
  getCanvasHost,
...
import {
  ...
  readInjectionDraft,
  readLiveElementDraft,
  applyCanvasDraftStyles,
  clearCanvasDraftStyles,
} from './chef/drafts'
import {
  ensureFroamNodeId,
  isStructuralLayerElement,
  syncStructureBoundaryLabel,
  buildLayerNode,
  collectLayers,
} from './chef/layers'
import {
...
export default function GlobalChefEditor({ initialOpen = false, routeKey: explicitRouteKey, projectKey: explicitProjectKey }: GlobalChefEditorProps) {
  ...
  const [workspaceActivity, setWorkspaceActivity] = useState<'scanning' | 'screenshot' | 'mutating' | 'chaos' | 'synthetic' | 'intent-understanding' | 'intent-creating' | 'intent-applying' | null>(null)
  ...
  const refreshNotes = useCallback(async () => {
    ...
    try {
      ...
      if (room.role === 'owner' || room.role === 'editor' || room.role === 'contributor') setRequests(await room.client.requests())
    } catch { /* offline */ }
  ...
  useEffect(() => {
    if (room.events.some((event) => event.type === 'comment' || event.type === 'revision' || event.type === 'request')) void refreshNotes()
  ...
  const readRequestOps = (): Record<string, string[]> => {
    if (!requestOpsKey) return {}
    try { return JSON.parse(window.localStorage.getItem(requestOpsKey) ?? '{}') as Record<string, string[]> } catch { return {} }
  }
  const excludedRequestOps = useMemo(() => {
    ...
    for (const request of requests) {
      ...
      for (const id of byRequest[request.id] ?? []) held.add(id)
    }
    return held
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const submitChangeRequest = useCallback(async (title: string, note: string) => {
    if (!room.client || !contributorRequest || !contributorRequest.changes.length) return false
    try {
      ...
      if (request && requestOpsKey) {
        try { window.localStorage.setItem(requestOpsKey, JSON.stringify({ ...readRequestOps(), [request.id]: contributorRequest.opIds })) } catch { /* private mode */ }
      }
      await refreshNotes()
  ...
  const withdrawChangeRequest = useCallback(async (request: RoomRequest) => {
    if (!room.client) return
    try {
  ...
  const applyRequestToStore = (base: EditorStore, request: RoomRequest): EditorStore => {
    const key = `${request.routeKey}@@${request.viewport}`
    const route = { ...(base[key] ?? {}) } as Record<string, ElementDraft>
    for (const [path, draft] of Object.entries(request.store)) route[path] = draft as ElementDraft
    for (const path of request.removed) delete route[path]
    return { ...base, [key]: route }
  }

  const previewChangeRequest = useCallback((request: RoomRequest | null) => {
    if (!request) {
  ...
  const decideChangeRequest = useCallback(async (request: RoomRequest, decision: 'approved' | 'changes-requested', note: string) => {
    if (!room.client) return
    ...
    try {
      ...
      if (decision === 'approved' && decided) {
        ...
        try {
          const loaded = await window.fetch(bridgeUrl('/__froam/repo/load')).then((response) => response.json()) as { design?: { routes?: Record<string, Record<string, Record<string, ElementDraft>>> } }
          ...
          if (written) {
            ...
            for (const path of [...Object.keys(decided.store), ...decided.removed]) {
              if (written[path]) route[path] = written[path]
              else delete route[path]
            }
            next = { ...next, [key]: route }
          }
  ...
  const inviteLinks = useMemo(() => {
    ...
    if (!owned) return {}
    const link = (role: 'editor' | 'contributor' | 'commenter' | 'viewer') => (owned.invites[role] ? room.inviteLink(owned, role) : undefined)
  ...
  function startWriting(target: HTMLElement, caret: CaretTarget = 'end') {
    const root = getRoot()
    if (!root || !isInPageScope(target, root) || !isWritableElement(target)) return false
  ...
  useEffect(() => {
    if (resetForRouteRef.current === routeKey) return
  ...
  function previewConnectedCanvas(next: EditorStore | null, protectRollback = true) {
  ...
  function archiveCurrentSelection(kind: 'component' | 'style' | 'motion' | 'interface-pattern') {
    if (!selection?.nodeId || !currentSelectionRef.current) return showToast('Select an element before adding it to Archive')
    ...
    const item = createArchiveItem({
      ...
      includes: kind === 'interface-pattern' ? ['structure', 'styles', ...(interaction ? ['motion', 'behavior'] as const : [])] : kind === 'component' ? ['structure'] : kind === 'style' ? ['styles'] : ['motion', 'behavior'],
  ...
  function applyStoreToDOM(targetStore: EditorStore, options: { clearCurrent?: boolean; previousStore?: EditorStore } = {}) {
    const root = getRoot()
    if (!root) return
   ...
   */
  function applyPseudoStyle(pseudo: PseudoElement, styles: Record<string, string>, label: string) {
  ...
  function selectElementFromIntel(el: HTMLElement) {
    const root = getRoot()
    if (!root || !isInPageScope(el, root)) return
  ...
  async function publishForSession() {
    ...
    if (room.role === 'contributor') return
    try {
      const routeSnapshot = collectVersionRouteDrafts()
   ...
   */
  async function writeCopyToSource(drafts: Record<string, ElementDraft>) {
    const originals = originalsRef.current[viewportStoreKeyRef.current] ?? {}
    const edits: Array<{ path: string; from: string; to: string }> = []
    for (const [path, draft] of Object.entries(drafts)) {
      if (typeof draft.text !== 'string' || !isSafeDraftPath(path)) continue
      ...
      if (!from || from.trim() === draft.text.trim()) continue
      edits.push({ path, from, to: draft.text })
    }
    if (!edits.length) return { drafts, note: '' }
    try {
      ...
      const data = await response.json().catch(() => null) as { success?: boolean; results?: Array<{ status: string; file?: string }> } | null
      if (!response.ok || !data?.success || !Array.isArray(data.results)) return { drafts, note: '' }
      const next = { ...drafts }
      const files = new Set<string>()
      data.results.forEach((result, index) => {
        const edit = edits[index]
        if (!edit || result.status !== 'written') return
        const { text: _written, ...rest } = next[edit.path] ?? {}
        if (rest.styles || rest.imageUrl !== undefined) next[edit.path] = rest
        else delete next[edit.path]
    ...
    try {
      ...
      if (root) {
        ...
        document.querySelectorAll('[data-chef-selected="true"], [data-froam-writable]').forEach((el) => {
          if (!isInPageScope(el, root)) return
  ...
  function placeInsertedNode(node: HTMLElement, placement: FroamInsertPlacement, target?: HTMLElement | null) {
    const root = getRoot()
  ...
  function insertLibraryComponent(componentId: string, placement: FroamInsertPlacement, frame: FroamFrameSpec, target?: HTMLElement | null) {
    ...
    if (!placeInsertedNode(node, placement, target)) return
    selectInsertedElement(node)
    persistLiveRouteSnapshot()
  ...
  function announceArrival(node: HTMLElement) {
  ...
  function executeAskFroam() {
    ...
    if (!intent || !askFroamVisible) return
    setCommandPaletteOpen(false)
    setCommandSearch('')
  ...
  return createPortal(
    <>
      ...
      <FroamBlueprint
        ...
        onOpenLayers={() => openWorkspaceSection('layers', 'understand')}
      />
        ...
        >
          <div className="fs-command-palette__card" data-chef-editor-root="true">
            <input
              ...
              type="text"
              value={commandSearch}
              placeholder="Type a command or ask Froam…"
              aria-label="Search commands or ask Froam"
              role="combobox"
              aria-controls="froam-command-results"
              aria-expanded="true"
              aria-activedescendant={filteredCommands[commandFocusIndex] ? `froam-command-${filteredCommands[commandFocusIndex].id}` : askFroamVisible && commandFocusIndex === 0 ? 'froam-command-ask' : undefined}
              autoFocus
              onChange={(e) => { setCommandSearch(e.target.value); setCommandFocusIndex(0) }}
              onKeyDown={(e) => {
                if (e.key === 'ArrowDown') { e.preventDefault(); setCommandFocusIndex((i) => Math.min(i + 1, Math.max(0, commandResultCount - 1))) }
                ...
                else if (e.key === 'Enter' && askFroamVisible) executeAskFroam()
            ...
            <ul className="fs-command-palette__list" id="froam-command-results" role="listbox">
              ...
              {filteredCommands.length === 0 && !askFroamVisible && (
        ...
        >
          <FroamSectionBoundary name="Toolbar">
            <FroamToolbar
              ...
              onSaveRepo={isContributor ? undefined : () => { void actionsRef.current.saveToRepo() }}
              collaborate={(
                <FroamCollaborate
                  role={room.role}
                  ...
                  requests={requests}
                  pendingChanges={contributorRequest?.changes ?? []}
                ...
                <div className="froam-figma-left__view">
                  <FroamSectionBoundary name="SitePlanner">
                    <FroamSitePlanner
                      ...
                      onToast={showToast}
                      sampleTheme={() => sampleSiteTheme(getRoot() ?? undefined)}
                    />
                  </FroamSectionBoundary>
                </div>
              ) : null}
      ...
      <FroamIntelligence
        ...
        onSelectNode={selectConnectedNode}
        onInsertArchived={insertArchivedHtml}
        onApplyArchivedStyle={(styles) => applyStyle(styles, undefined, 'Archive style')}
      ...
      <FroamContextMenu
        ...
        onAction={(action) => {
          switch (action) {
            ...
            case 'archive-motion': archiveCurrentSelection('motion'); break
            case 'archive-pattern': archiveCurrentSelection('interface-pattern'); break
          }
        }}
