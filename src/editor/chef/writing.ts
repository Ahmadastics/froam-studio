export function canApplyTextDraft(element: HTMLElement) {
...
export function isWritableElement(element: Element | null | undefined): element is HTMLElement {
...
export type CaretTarget = 'end' | { x: number; y: number; word?: boolean }

/** Put the caret where the person pointed (or select the word there), else at the end. */
export function placeCaret(element: HTMLElement, caret: CaretTarget) {
...
export function isEditableField(target: EventTarget | null) {
  if (!(target instanceof HTMLElement)) return false
  return target.isContentEditable || ['INPUT', 'TEXTAREA', 'SELECT'].includes(target.tagName)
}

export function isTextVisualLayer(element: HTMLElement) {
