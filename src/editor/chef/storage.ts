import { clearOpLog } from '../../collab/persist'
import { isSafeDraftPath } from '../../collab/paths'
import { type FroamNodeRegistry } from '../../project/node-registry'
import { froamStorageKey } from '../../project/storage-scope'
import { type BrandFont, sanitizeBrandFonts } from '../fontSources'
import {
...
import { type ElementDraft, type EditorStore, CANVAS_KEY } from './types'
...
export function loadBrandFonts(projectKey: string): BrandFont[] {
...
export function saveBrandFontsForProject(fonts: BrandFont[], projectKey: string) {
  if (typeof window === 'undefined') return
  try {
...
export function loadStore(projectKey: string): EditorStore {
  if (typeof window === 'undefined') return {}
  try {
...
export function saveStoreForProject(store: EditorStore, projectKey: string) {
  if (typeof window === 'undefined') return
...
export function loadNodeRegistry(projectKey: string): FroamNodeRegistry {
  try {
...
export function saveNodeRegistryForProject(registry: FroamNodeRegistry, projectKey: string) {
  try {
...
export function savePersonaPreference(persona: FroamPersona) {
  if (typeof window === 'undefined') return
  try {
...
export function personasEqual(left: FroamPersona, right: FroamPersona) {
  return left.name === right.name
    && left.tagline === right.tagline
    && left.imageUrl === right.imageUrl
}

export function stripPersonaDrafts(drafts: Record<string, ElementDraft>) {
  const nextDrafts = { ...drafts }
  delete nextDrafts[FROAM_PERSONA_PATH]
  return nextDrafts
}

export function withPersonaDraft(drafts: Record<string, ElementDraft>, persona: FroamPersona) {
  return {
...
export function countRenderableDrafts(drafts: Record<string, ElementDraft>) {
  return Object.keys(drafts).filter((path) => !isFroamPersonaPath(path)).length
}

export function sanitizeStore(store: EditorStore): EditorStore {
