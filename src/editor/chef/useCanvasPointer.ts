...
import { getElementPath, isInPageScope } from '../../collab/paths'
import { buildSelection, getRoot } from './dom'
import { createHitTester } from './hit-test'
import { buildLayerNode, syncStructureBoundaryLabel } from './layers'
import type { FroamToolMode, LayerNode, SelectionState } from './types'
import { isWritableElement, type CaretTarget } from './writing'
...
export type CanvasPointerOptions = {
  ...
  lastClickPointRef: Ref<{ x: number; y: number } | null>
  panelOpenRef: Ref<boolean>
  selectionRef: Ref<SelectionState | null>
  selectionsRef: Ref<SelectionState[]>
  setActive: Setter<boolean>
  setClickPulse: Setter<{ key: number; x: number; y: number; rect: DOMRect } | null>
 ...
 */
export function useCanvasPointer(options: CanvasPointerOptions) {
  const {
