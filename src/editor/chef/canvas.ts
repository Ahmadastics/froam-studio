...
import { getRoot, getCanvasHost, rgbToHex, readImageUrl } from './dom'

export function collectCSSVars(): CSSVarEntry[] {
...
export async function capturePageThumb(): Promise<string | null> {
  try {
    const root = getRoot()
    if (!root) return null
...
export function syncFroamArtboardMetadata(element: HTMLElement) {
...
export function buildGradientCSS(type: 'linear' | 'radial', angle: number, stops: GradientStop[]) {
