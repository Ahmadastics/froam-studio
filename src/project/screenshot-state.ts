...
export type FroamScreenshotStateDifference = {
  ...
  geometryChanges: Array<{ fromNodeId: string; toNodeId: string; distance: number }>
  styleChanges: Array<{ fromNodeId: string; toNodeId: string; colorChanged: boolean }>
  interactionHypotheses: Array<{ kind: 'reveal' | 'hide' | 'move' | 'restyle'; confidence: number; evidence: string[] }>
  limitations: string[]
}

function clamp(value: number) { return Math.max(0, Math.min(1, value)) }
function normalizedText(value?: string) { return value?.toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim() ?? '' }
function tokens(value?: string) { return new Set(normalizedText(value).split(' ').filter(Boolean)) }
...
export function normalizedScreenshotGeometry(region: FroamScreenshotRegion, viewport: { width: number; height: number }) {
  const width = Math.max(1, viewport.width); const height = Math.max(1, viewport.height)
...
function parentSignature(reconstruction: FroamScreenshotReconstruction, region: FroamScreenshotRegion) {
...
function candidateScore(
  source: FroamScreenshotRegion,
  target: FroamScreenshotRegion,
  sourceIndex: number,
  targetIndex: number,
  from: FroamScreenshotReconstruction,
  to: FroamScreenshotReconstruction,
  fromViewport: { width: number; height: number },
  toViewport: { width: number; height: number },
) {
  const evidence: FroamScreenshotMatchEvidence[] = []
  let score = 0
  if (source.kind === target.kind) { score += .16; evidence.push({ signal: 'kind', score: .16, detail: `same ${source.kind} region kind` }) }
  if (source.semanticRole !== 'unknown' && source.semanticRole === target.semanticRole) { score += .18; evidence.push({ signal: 'semantic-role', score: .18, detail: `same ${source.semanticRole} role` }) }
  const sourceText = normalizedText(source.text); const targetText = normalizedText(target.text)
  if (sourceText && sourceText === targetText) { score += .34; evidence.push({ signal: 'ocr-text', score: .34, detail: 'exact normalized OCR text' }) }
  else {
    const textScore = tokenSimilarity(source.text, target.text) * .24
    if (textScore > .04) { score += textScore; evidence.push({ signal: 'ocr-text', score: textScore, detail: 'overlapping OCR tokens' }) }
  }
  const a = normalizedScreenshotGeometry(source, fromViewport); const b = normalizedScreenshotGeometry(target, toViewport)
  ...
  if (orderSimilarity > .7) evidence.push({ signal: 'ordering', score: orderSimilarity * .05, detail: 'similar reading order' })
  if (source.componentFamilyId && source.componentFamilyId === target.componentFamilyId) { score += .08; evidence.push({ signal: 'component-family', score: .08, detail: 'same reconstructed component family' }) }
  if (source.averageColor && source.averageColor === target.averageColor) { score += .04; evidence.push({ signal: 'visual', score: .04, detail: 'same sampled average color' }) }
  const sourceParent = parentSignature(from, source); const targetParent = parentSignature(to, target)
  if (sourceParent && sourceParent === targetParent) { score += .05; evidence.push({ signal: 'hierarchy', score: .05, detail: 'compatible reconstructed parent role' }) }
 ...
 */
export function matchScreenshotRegions(from: FroamScreenshotReconstruction, to: FroamScreenshotReconstruction): FroamScreenshotRegionMatching {
  ...
  const used = new Set<string>(); const matches: FroamScreenshotRegionMatch[] = []; const ambiguous: FroamScreenshotAmbiguousMatch[] = []
  for (const proposal of proposals) {
    const available = proposal.candidates.filter((candidate) => !used.has(candidate.target.id))
    const best = available[0]; const second = available[1]
    if (!best) continue
    ...
    if (best.confidence < .58) continue
...
export function compareScreenshotStates(from: FroamScreenshotReconstruction, to: FroamScreenshotReconstruction): FroamScreenshotStateDifference {
  ...
  const fromById = new Map(from.regions.map((region) => [region.id, region])); const toById = new Map(to.regions.map((region) => [region.id, region]))
  const appeared = matching.unmatchedToRegionIds.map((id) => toById.get(id)).filter((item): item is FroamScreenshotRegion => Boolean(item))
  const disappeared = matching.unmatchedFromRegionIds.map((id) => fromById.get(id)).filter((item): item is FroamScreenshotRegion => Boolean(item))
  const geometry = matching.matches.flatMap((match) => { const source = fromById.get(match.fromRegionId); const target = toById.get(match.toRegionId); if (!source || !target) return []; const distance = Math.hypot(source.x / Math.max(1, from.references[0]?.width ?? 1) - target.x / Math.max(1, to.references[0]?.width ?? 1), source.y / Math.max(1, from.references[0]?.height ?? 1) - target.y / Math.max(1, to.references[0]?.height ?? 1)); return distance > .02 ? [{ fromNodeId: source.nodeId, toNodeId: target.nodeId, distance }] : [] })
  const styles = matching.matches.flatMap((match) => { const source = fromById.get(match.fromRegionId); const target = toById.get(match.toRegionId); return source && target && source.averageColor !== target.averageColor ? [{ fromNodeId: source.nodeId, toNodeId: target.nodeId, colorChanged: true as const }] : [] })
  const hypotheses: FroamScreenshotStateDifference['interactionHypotheses'] = []
  if (appeared.length) hypotheses.push({ kind: 'reveal', confidence: Math.min(.75, .4 + appeared.length / Math.max(1, to.regions.length) * .3), evidence: [`${appeared.length} unmatched regions appeared`] })
  if (disappeared.length) hypotheses.push({ kind: 'hide', confidence: Math.min(.75, .4 + disappeared.length / Math.max(1, from.regions.length) * .3), evidence: [`${disappeared.length} unmatched regions disappeared`] })
  if (geometry.length) hypotheses.push({ kind: 'move', confidence: .55, evidence: [`${geometry.length} matched regions moved in normalized geometry`] })
  if (styles.length) hypotheses.push({ kind: 'restyle', confidence: .5, evidence: [`${styles.length} matched regions changed sampled color`] })
...
export function inferResponsiveScreenshotReferences(reconstructions: readonly FroamScreenshotReconstruction[]) {
