...
export function deriveBranchState(document: FroamProjectDocument, branchId = document.activeBranchId): FroamProjectState {
...
export function renameProjectBranch(document: FroamProjectDocument, branchId: FroamId, name: string, now = Date.now()) {
  const branch = document.branches[branchId]
  if (!branch) throw new Error(`Unknown Froam branch: ${branchId}`)
  const cleanName = name.trim().slice(0, 80)
  if (!cleanName) throw new Error('A Froam prototype needs a name')
  return {
    ...document,
    updatedAt: now,
...
export function deleteProjectBranch(document: FroamProjectDocument, branchId: FroamId, now = Date.now()) {
  const branch = document.branches[branchId]
  if (!branch) throw new Error(`Unknown Froam branch: ${branchId}`)
  if (branchId === 'main' || branch.parentBranchId === null) throw new Error('The main Froam branch cannot be deleted')
  ...
  for (const checkpoint of Object.values(checkpoints)) {
    if (checkpoint.branchId === branchId) delete checkpoints[checkpoint.id]
  }
  return {
    ...document,
