 ...
 *    are not comparable.
 */
import { FROAM_MIN_TARGET_PX, FROAM_MIN_TYPE_PX } from './page-profile'
...
export type FroamJudgeVerdict = {
  ...
  deltas: Array<{ id: string; before: number; after: number; delta: number; weight: number }>
 ...
 */
export const FROAM_JUDGE_WEIGHTS: Record<string, number> = {
  contrast: 3,
  'touch-targets': 2,
  'spacing-grid': 2,
  'token-drift': 2,
...
export function confidenceForMargin(margin: number, band = FROAM_JUDGE_EQUIVALENCE_BAND) {
  if (margin < band) return round(clamp01(1 - margin / band))
...
function band(value: number, low: number, high: number, softness: number) {
...
function checkContrast(profile: FroamPageProfile): FroamJudgeCheckResult {
  ...
  const findings: FroamJudgeFinding[] = contrastFailures.slice(0, 20).map((pair) => ({
  ...
  if (contrastSamples === 0) return { id: 'contrast', score: 1, weight: FROAM_JUDGE_WEIGHTS.contrast, normative: true, findings: [] }
  return {
    id: 'contrast',
...
function checkTouchTargets(profile: FroamPageProfile): FroamJudgeCheckResult {
  const { targetSamples, undersizedTargets, smallestTargetPx, exemptInlineTargets } = profile.interaction
  if (targetSamples === 0) return { id: 'touch-targets', score: 1, weight: FROAM_JUDGE_WEIGHTS['touch-targets'], normative: true, findings: [] }
  const failureRate = undersizedTargets.length / targetSamples
  const findings: FroamJudgeFinding[] = undersizedTargets.slice(0, 20).map((target) => ({
...
function checkAccessibilityWarnings(profile: FroamPageProfile): FroamJudgeCheckResult {
...
function checkSpacingGrid(profile: FroamPageProfile): FroamJudgeCheckResult {
...
function checkTokenDrift(profile: FroamPageProfile): FroamJudgeCheckResult {
...
function checkAccentDiscipline(profile: FroamPageProfile): FroamJudgeCheckResult {
...
function checkTypeScale(profile: FroamPageProfile): FroamJudgeCheckResult {
  ...
  const findings: FroamJudgeFinding[] = []
  if (steps > 7) findings.push({
    id: 'type-scale:steps',
    check: 'type-scale',
  ...
  if (profile.type.belowMinimumSizes > 0) findings.push({
  ...
  if (profile.type.ratio === null && steps > 2) findings.push({
    id: 'type-scale:ratio',
    check: 'type-scale',
    severity: 'minor',
...
function checkMeasure(profile: FroamPageProfile): FroamJudgeCheckResult {
  const measure = profile.type.measureCh
  if (!measure) return { id: 'measure', score: 1, weight: FROAM_JUDGE_WEIGHTS.measure, normative: false, findings: [] }
  return {
    id: 'measure',
...
function checkRadiusConsistency(profile: FroamPageProfile): FroamJudgeCheckResult {
...
function checkShadowTiers(profile: FroamPageProfile): FroamJudgeCheckResult {
...
function checkComponentVariance(profile: FroamPageProfile): FroamJudgeCheckResult {
  const families = profile.components.filter((family) => family.instances >= 2)
  if (!families.length) return { id: 'component-variance', score: 1, weight: FROAM_JUDGE_WEIGHTS['component-variance'], normative: false, findings: [] }
...
const CHECKS: Array<(profile: FroamPageProfile) => FroamJudgeCheckResult> = [
  checkContrast,
  checkTouchTargets,
 ...
 */
export function auditProfile(profile: FroamPageProfile): FroamPageAudit {
  ...
  const severityRank: Record<FroamJudgeSeverity, number> = { veto: 0, major: 1, minor: 2, info: 3 }
  return {
    schemaVersion: 1,
...
function comparability(before: FroamPageProfile, after: FroamPageProfile): string | null {
  if (before.viewport !== after.viewport) return `viewports differ (${before.viewport} vs ${after.viewport}); profiles are not comparable`
 ...
 */
function findVeto(before: FroamPageProfile, after: FroamPageProfile, beforeChecks: FroamJudgeCheckResult[], afterChecks: FroamJudgeCheckResult[]): FroamJudgeFinding | null {
 ...
 */
export function compareProfiles(before: FroamPageProfile, after: FroamPageProfile): FroamJudgeVerdict {
