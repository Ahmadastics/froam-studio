...
export type FroamScreenshotReferenceMeta = { viewportWidth?: number; viewportHeight?: number; state?: string; route?: string; label?: string; realityResearch?: boolean; sourceWidth?: number; sourceHeight?: number; quad?: unknown; limitations?: string[] }
...
function containsRegion(parent: FroamScreenshotRegion, child: FroamScreenshotRegion) { return parent.id !== child.id && parent.x <= child.x && parent.y <= child.y && parent.x + parent.width >= child.x + child.width && parent.y + parent.height >= child.y + child.height && parent.width * parent.height > child.width * child.height }
...
export function createLocalScreenshotProvider(ocrProvider: FroamOcrProvider = browserTextDetectorOcrProvider): FroamScreenshotProvider {
  return { id: 'froam-local-reconstruction-v2', local: true, async reconstruct(input) {
    ...
    for (const region of regions) { const parentRegion = regions.filter((candidate) => containsRegion(candidate, region)).sort((a, b) => a.width * a.height - b.width * b.height)[0]; const parentId = parentRegion?.nodeId ?? rootNodeId; nodes.push({ id: region.nodeId, kind: 'element', name: region.text || region.semanticRole || region.kind, parentId, source: 'imported', metadata: { reconstructionRegion: region, componentFamilyId: region.componentFamilyId, hierarchyInferred: true } }); relations.push({ id: `contains:${parentId}:${region.nodeId}`, kind: 'contains', from: parentId, to: region.nodeId, metadata: { inferred: true, confidence: parentRegion ? .58 : .9 } }); if (region.componentFamilyId) relations.push({ id: `belongs:${region.nodeId}:${region.componentFamilyId}`, kind: 'belongs-to', from: region.nodeId, to: region.componentFamilyId }); dna.push({ schemaVersion: 1, nodeId: region.nodeId, capturedAt: now, identity: { source: 'screenshot', stable: true }, structure: { parentId, componentFamilyId: region.componentFamilyId, hierarchyInferred: true }, layout: { position: 'absolute', x: region.x, y: region.y, width: region.width, height: region.height }, visual: { averageColor: region.averageColor }, semantics: { role: region.semanticRole ?? region.kind, confidence: region.textConfidence ?? region.confidence, text: region.text, uncertain: region.textConfidence !== undefined && region.textConfidence < .5 }, responsive: { referenceMetadata: primary.metadata }, provenance: { source: 'screenshot', provider: this.id, referenceId: primary.referenceId } }) }
