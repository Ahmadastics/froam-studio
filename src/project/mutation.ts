...
function proposalPayloadNodeIds(proposal: FroamMutationProposal): string[] | null {
  const payload = proposal.payload as Record<string, any>
  if (proposal.type === 'node.upserted') return typeof payload.node?.id === 'string' ? [payload.node.id] : null
  if (proposal.type === 'relation.upserted') return typeof payload.relation?.from === 'string' && typeof payload.relation?.to === 'string' ? [payload.relation.from, payload.relation.to] : null
  if (proposal.type === 'interaction.upserted') return typeof payload.interaction?.sourceId === 'string' && Array.isArray(payload.interaction?.targetIds) ? [payload.interaction.sourceId, ...payload.interaction.targetIds] : null
  if (proposal.type === 'dna.captured') return typeof payload.dna?.nodeId === 'string' ? [payload.dna.nodeId] : null
  if (proposal.type === 'responsive.upserted') return typeof payload.responsive?.nodeId === 'string' ? [payload.responsive.nodeId] : null
  return null
}
function proposalAllowed(request: FroamMutationRequest, proposal: FroamMutationProposal) {
  ...
  if (!proposal.targetIds.length || proposal.targetIds.some((id) => !scope.has(id) || !request.state.nodes[id])) return false
  if (!payloadNodeIds || payloadNodeIds.some((id) => !scope.has(id) || !request.state.nodes[id] || isProtected(request, id))) return false
  if (!payloadNodeIds.some((id) => proposal.targetIds.includes(id))) return false
  if (!Number.isFinite(proposal.confidence) || typeof proposal.rationale !== 'string' || !proposal.rationale.trim()) return false
 ...
 */
function createLineageRelations(input: { project: FroamProjectDocument; branchId: string; actorId: string; scopeNodeIds: string[]; state: FroamProjectState; baseClock: number; priorEventCount: number; now: number; sourceBranchId: string; sourceCheckpointId: string; level: FroamMutationLevel; provider: string; idFactory?: () => string }) {
...
export function createMutationPrototype(document: FroamProjectDocument, input: { branchId: string; name?: string; actorId: string; level: FroamMutationLevel; scopeNodeIds: string[]; provider?: FroamMutationProvider; constraints?: Partial<FroamMutationConstraints>; projectContext?: Record<string, unknown>; now?: number; seed?: number; idFactory?: () => string }) {
  ...
  return { project: { ...project, metadata: { ...project.metadata, mutations: [...((project.metadata?.mutations as FroamMutationProvenance[] | undefined) ?? []), completeProvenance] } }, provenance: completeProvenance, proposals: preview.proposals, preview }
}

function stable(value: unknown) { return JSON.stringify(value) }
function storeField(state: FroamProjectState, op: FroamOp) {
...
export function compareMutationBranches(document: FroamProjectDocument, sourceBranchId: string, mutationBranchId: string): FroamMutationComparison {
  ...
  const provenance = ((document.metadata?.mutations as FroamMutationProvenance[] | undefined) ?? []).find((item) => item.id === mutationBranchId)
...
function safeCompiledStyle(value: unknown) {
  ...
  if (!trimmed || trimmed.length > 300 || /url\s*\(|expression\s*\(|javascript:|[{};]/i.test(trimmed)) return null
  return trimmed
}
function proposalStyleRecord(proposal: FroamMutationProposal) {
  if (proposal.type !== 'dna.captured') return null
  const dna = (proposal.payload as { dna?: FroamDNA }).dna
  if (!dna) return null
...
function compileMutationDesignEvents(input: { project: FroamProjectDocument; branchId: string; actorId: string; proposals: FroamMutationProposal[]; snapshot: FroamMutationSelectionSnapshot; baseClock: number; now: number; preserveDimensions: boolean; idFactory?: () => string }) {
  ...
  for (const proposal of input.proposals) {
    ...
    for (const property of COMPILED_STYLE_FIELDS[proposal.domain]) {
      if (input.preserveDimensions && DIMENSION_SENSITIVE_STYLES.has(property)) continue
      const value = safeCompiledStyle((record as Record<string, unknown>)[property])
      if (!value || value === baseline?.[property] || compiled.some((item) => item.property === property)) continue
      compiled.push({ property, value, rationale: proposal.rationale })
      if (compiled.length >= 12) break
    }
    if (compiled.length >= 12) break
  }
 ...
 */
export function createMutationPrototypeFromProposals(
  ...
  const state = input.selectionSnapshot ? {
    ...
    relations: Object.fromEntries([
      ...
      ...(input.selectionSnapshot.relationships ?? []).map((relation) => [relation.id, relation] as const),
    ]),
  } : sourceState
  const constraints = normalizeMutationConstraints(input.level, input.constraints)
  const request: FroamMutationRequest = {
    state,
  ...
  if (allowed.length === 0) throw new Error('No safe mutation proposals')
  ...
  const observationEvents: FroamProjectEvent[] = []
  if (input.selectionSnapshot) {
    ...
    if (snapshot.scan) observationEvents.push(createProjectEvent({ projectId: project.id, branchId: input.branchId, actorId: input.actorId, clock: ++baseClock, createdAt: now, type: 'scan.captured', payload: { scan: snapshot.scan }, targetIds: [snapshot.node.id], label: 'Intent scan evidence', idFactory: input.idFactory }))
    if (snapshot.dna) observationEvents.push(createProjectEvent({ projectId: project.id, branchId: input.branchId, actorId: input.actorId, clock: ++baseClock, createdAt: now, type: 'dna.captured', payload: { dna: snapshot.dna }, targetIds: [snapshot.node.id], label: 'Intent DNA evidence', idFactory: input.idFactory }))
    for (const relation of (snapshot.relationships ?? []).slice(0, 16)) observationEvents.push(createProjectEvent({ projectId: project.id, branchId: input.branchId, actorId: input.actorId, clock: ++baseClock, createdAt: now, type: 'relation.upserted', payload: { relation }, targetIds: [relation.from, relation.to], label: 'Intent relationship evidence', idFactory: input.idFactory }))
    project = appendProjectEvents(project, observationEvents)
  }
  const events = allowed.map((proposal, index) =>
    createProjectEvent({
  ...
  return {
    project: {
      ...
      metadata: {
        ...project.metadata,
        mutations: [...((project.metadata?.mutations as FroamMutationProvenance[] | undefined) ?? []), completeProvenance],
      },
    },
