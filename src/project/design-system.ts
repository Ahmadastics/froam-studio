...
export function emptyDesignSystem(): FroamDesignSystem {
  const createdAt = now()
  const modes: FroamDesignMode[] = [
...
export function normalizeDesignSystem(value?: Partial<FroamDesignSystem>): FroamDesignSystem {
...
function modeChain(system: FroamDesignSystem, modeId: string) {
  const result: string[] = []
  const seen = new Set<string>()
  let cursor: string | undefined = modeId
  while (cursor && !seen.has(cursor)) {
    seen.add(cursor); result.push(cursor); cursor = system.modes[cursor]?.parentId
  }
  if (!result.includes('mode:base')) result.push('mode:base')
  return result
}

export function resolveDesignVariable(system: FroamDesignSystem, variableId: string, modeIds = system.activeModeIds, seen = new Set<string>()): string | undefined {
  if (seen.has(variableId)) return undefined
  const variable = system.variables[variableId]
  if (!variable) return undefined
  seen.add(variableId)
  if (variable.aliasTo) return resolveDesignVariable(system, variable.aliasTo, modeIds, seen)
  for (const modeId of modeIds) for (const candidate of modeChain(system, modeId)) {
    if (variable.values[candidate] !== undefined) return variable.values[candidate]
  }
  return variable.values['mode:base'] ?? Object.values(variable.values)[0]
}

export function upsertDesignVariable(system: FroamDesignSystem, variable: FroamDesignVariable) {
  return { ...system, variables: { ...system.variables, [variable.id]: variable } }
}

export function setActiveModes(system: FroamDesignSystem, modeIds: string[]) {
  const valid = [...new Set(modeIds)].filter((id) => Boolean(system.modes[id]))
  return { ...system, activeModeIds: valid.length ? valid : ['mode:base'] }
}

export function designVariableCss(system: FroamDesignSystem, modeIds = system.activeModeIds) {
...
export function createReusableStyle(input: { id: string; name: string; category?: string; states: Partial<Record<FroamStyleState, Record<string, string>>>; now?: number }): FroamReusableStyle {
...
export function saveReusableStyle(system: FroamDesignSystem, style: FroamReusableStyle) {
...
export function recordStyleUse(system: FroamDesignSystem, styleId: string, nodeId: string) {
...
export function upsertComponentFamily(system: FroamDesignSystem, family: FroamComponentFamily) {
...
export function createSiteKit(input: Omit<FroamSiteKit, 'createdAt' | 'updatedAt' | 'version'> & { now?: number }): FroamSiteKit {
  const createdAt = input.now ?? now()
  return { ...input, createdAt, updatedAt: createdAt, version: 1 }
}

export function upsertSiteKit(system: FroamDesignSystem, kit: FroamSiteKit) {
...
export function installLibrary(system: FroamDesignSystem, library: FroamLibrary) {
  return { ...system, libraries: { ...system.libraries, [library.id]: library } }
}

export function publishLibraryRelease(system: FroamDesignSystem, libraryId: string, notes?: string, at = now()) {
  ...
  return { ...system, libraries: { ...system.libraries, [libraryId]: { ...library, availableVersion: version, status: 'update-available' as const, releases: [...library.releases, release], updatedAt: at } } }
}

export function decideLibraryUpdate(system: FroamDesignSystem, libraryId: string, decision: 'accept' | 'postpone', at = now()) {
  const library = system.libraries[libraryId]
  if (!library) return system
  const next = decision === 'accept'
    ? { ...library, installedVersion: library.availableVersion, status: 'current' as const, updatedAt: at }
    : { ...library, status: 'postponed' as const, updatedAt: at }
...
export function seedStarterDesignSystem(system = emptyDesignSystem(), at = now()) {
  if (Object.keys(system.variables).length || Object.keys(system.componentFamilies).length) return system
  const variables: FroamDesignVariable[] = [
