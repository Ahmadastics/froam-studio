...
function heuristicPrediction(records: readonly FroamScanRecord[], now = Date.now()): FroamAnalysis {
  const raw = records.map((record) => {
    ...
    const font = Number.parseFloat(String(visual.fontSize ?? 0)); if (font >= 28) { score += 18; reasons.push('large type') }
    const role = String(semantic.role ?? 'unknown'); if (['cta', 'button'].includes(role)) { score += 14; reasons.push('action affordance') }
    if (role === 'media') { score += 12; reasons.push('large visual region') }
    if (Number(layout.rect?.y ?? 9999) < 800) { score += 8; reasons.push('early viewport position') }
