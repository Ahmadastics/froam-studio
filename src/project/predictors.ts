...
export function runPredictor(predictor: FroamPredictor, tasks: readonly FroamPretextTask[]): FroamPretextPrediction[] {
 ...
 */
export function fitMajorityBaseline(trainingTasks: readonly FroamPretextTask[]): FroamPredictor {
  const byKind = new Map<string, Map<string | number, number>>()
  for (const task of trainingTasks) {
    const counts = byKind.get(task.kind) ?? new Map<string | number, number>()
    counts.set(task.answer, (counts.get(task.answer) ?? 0) + 1)
    byKind.set(task.kind, counts)
  }
  const modal = new Map<string, string | number>()
  ...
  for (const [kind, counts] of byKind) {
    const values = [...counts.keys()].filter((value): value is number => typeof value === 'number')
...
export function uniformBaseline(seed = 1): FroamPredictor {
  return {
    name: 'uniform',
    predict(task) {
      if (!task.choices || !task.choices.length) return null
      let hash = seed >>> 0
      for (let index = 0; index < task.id.length; index += 1) hash = Math.imul(hash ^ task.id.charCodeAt(index), 0x01000193) >>> 0
      return task.choices[hash % task.choices.length]
    },
  }
}

// ── the fitted corpus predictor ─────────────────────────────────────────────

type ArchetypeCounts = Map<string, number>

const bump = (map: Map<string, ArchetypeCounts>, key: string, value: string) => {
  const counts = map.get(key) ?? new Map<string, number>()
  counts.set(value, (counts.get(value) ?? 0) + 1)
  map.set(key, counts)
}

const argmax = (counts: ArchetypeCounts | undefined) =>
  counts && counts.size ? [...counts].sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]))[0][0] : null

/** Position is bucketed so a 4-section page and a 9-section page can inform each other. */
const positionBucket = (index: number, total: number) => {
...
export type FroamCorpusModel = {
  ...
  geometry: Array<{ columns: number; heightRatio: number; isFirst: boolean; isLast: boolean; archetype: FroamSectionArchetype }>
  /** Accent role frequencies. */
  accentRoles: ArchetypeCounts
  /** Density examples keyed by type features. */
  densityExamples: Array<{ medianStep: number; measureCh: number; paletteSize: number; density: string }>
  /** Which interpolation fits observed type scales better, chosen on training data. */
  interpolation: 'geometric' | 'arithmetic'
  interpolationFit: { geometric: number; arithmetic: number }
  trainedOn: number
}
 ...
 */
export function fitCorpusModel(profiles: readonly FroamPageProfile[]): FroamCorpusModel {
  const transitions = new Map<string, ArchetypeCounts>()
  const positional = new Map<string, ArchetypeCounts>()
  const geometry: FroamCorpusModel['geometry'] = []
  const accentRoles: ArchetypeCounts = new Map()
  const densityExamples: FroamCorpusModel['densityExamples'] = []
  ...
  for (const profile of profiles) {
    ...
    sections.forEach((section, index) => {
      if (index > 0) bump(transitions, sections[index - 1].archetype, section.archetype)
    ...
    for (const role of accent?.appearsOn ?? []) if (role !== 'unknown') accentRoles.set(role, (accentRoles.get(role) ?? 0) + 1)
...
export function corpusPredictor(model: FroamCorpusModel): FroamPredictor {
  return {
    name: 'corpus',
    predict(task) {
      switch (task.kind) {
        case 'next-section': {
          const input = task.input as { prefix: string[] }
        ...
        case 'section-infill': {
          const input = task.input as { hiddenIndex: number; sectionCount: number; visible: Array<{ index: number; archetype: string }> }
        ...
        case 'blind-role': {
          const input = task.input as { columns: number; heightRatio: number; isFirst: boolean; isLast: boolean }
          ...
          const votes: ArchetypeCounts = new Map()
          for (const item of scored) votes.set(item.archetype, (votes.get(item.archetype) ?? 0) + 1)
          return argmax(votes)
        }
        case 'accent-placement':
          return argmax(model.accentRoles)
        case 'scale-infill': {
          const input = task.input as { hiddenIndex: number; visibleSteps: number[] }
        ...
        case 'density-match': {
          const input = task.input as { typeSteps: number[]; measureCh: number; paletteSize: number }
          if (!model.densityExamples.length) return null
          ...
          const votes: ArchetypeCounts = new Map()
          for (const item of scored) votes.set(item.density, (votes.get(item.density) ?? 0) + 1)
          return argmax(votes)
        }
        default:
          return null
...
export type FroamPredictorDiagnosis = {
  ...
  confusions: Array<{ expected: string; predicted: string; count: number }>
 ...
 */
export function diagnosePredictor(predictor: FroamPredictor, tasks: readonly FroamPretextTask[]): FroamPredictorDiagnosis[] {
  ...
  return kinds.map((kind) => {
    ...
    for (const task of forKind) {
      ...
      if (predicted === null) { abstained += 1; continue }
      if (isCorrect(task, predicted)) continue
