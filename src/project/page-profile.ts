...
export type FroamSectionArchetype =
  ...
  | 'pricing' | 'faq' | 'cta' | 'content' | 'footer' | 'unknown'
...
export function resolveSections(
  documentRoot: NodeView | undefined,
  ...
  const unwrap = (view: NodeView) => {
    let cursor = view
    for (let depth = 0; depth < SECTION_MAX_DEPTH; depth += 1) {
      if (SECTION_TAGS.has(cursor.tag)) return cursor
      const qualifying = childrenOf(cursor).filter(qualifies)
      if (qualifying.length !== 1) return cursor
      const only = qualifying[0]
      if (only.rect.height < cursor.rect.height * 0.9) return cursor
      cursor = only
    }
    return cursor
  }
  ...
  const tiling = (view: NodeView, children: NodeView[]) => {
    ...
    for (const child of ordered) {
      ...
      if (child.rect.y < previousBottom) overlap += Math.min(previousBottom - child.rect.y, child.rect.height)
...
function classifySection(childRoles: FroamSemanticRole[], input: {
  ...
  if (has('heading') && has('paragraph')) return { archetype: 'content', confidence: 0.45 }
  if (has('heading')) return { archetype: 'content', confidence: 0.3 }
...
export function buildPageProfile(input: FroamProfileInput): FroamPageProfile {
  ...
  const mergedPalette: FroamPaletteEntry[] = []
  for (const entry of palette) {
    const twin = mergedPalette.find((candidate) => candidate.role === entry.role
      && Math.hypot(
        rgbToOklab(oklabToRgb(oklchToOklab(candidate.oklch))).L - rgbToOklab(oklabToRgb(oklchToOklab(entry.oklch))).L,
        candidate.oklch.c * Math.cos(candidate.oklch.h * Math.PI / 180) - entry.oklch.c * Math.cos(entry.oklch.h * Math.PI / 180),
        candidate.oklch.c * Math.sin(candidate.oklch.h * Math.PI / 180) - entry.oklch.c * Math.sin(entry.oklch.h * Math.PI / 180),
      ) <= OKLAB_MERGE_THRESHOLD)
    if (!twin) { mergedPalette.push({ ...entry }); continue }
