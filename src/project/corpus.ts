...
import { auditProfile } from './judge-deterministic'
import type { FroamViewport } from '../collab/types'

export const FROAM_CORPUS_SCHEMA_VERSION = 1 as const
...
export const FROAM_LEARNABLE_TIERS: readonly FroamCorpusTier[] = ['reference', 'good']
...
export function emptyCorpus(): FroamCorpus {
  return { schemaVersion: FROAM_CORPUS_SCHEMA_VERSION, entries: [] }
}

const keyOf = (profile: FroamPageProfile) => `${profile.origin}|${profile.routeKey}|${profile.viewport}`
...
export function addToCorpus(
  corpus: FroamCorpus,
  profile: FroamPageProfile,
  input: { tier: FroamCorpusTier; tags?: string[]; note?: string; now?: number },
): FroamCorpus {
  const audit = auditProfile(profile)
  const entry: FroamCorpusEntry = {
    profile,
    tier: input.tier,
...
export function removeOrigin(corpus: FroamCorpus, origin: string): FroamCorpus {
...
export function selectEntries(corpus: FroamCorpus, filter: FroamCorpusFilter = {}): FroamCorpusEntry[] {
  return corpus.entries.filter((entry) => {
    if (filter.tiers && !filter.tiers.includes(entry.tier)) return false
    if (filter.viewport && entry.profile.viewport !== filter.viewport) return false
    if (filter.modeSignal && entry.profile.color.modeSignal !== filter.modeSignal) return false
    if (filter.origins && !filter.origins.includes(entry.profile.origin)) return false
    if (filter.tags && !filter.tags.every((tag) => entry.tags.includes(tag))) return false
    if (filter.archetype && !entry.profile.flow.sections.some((section) => section.archetype === filter.archetype)) return false
    return true
  })
}
 ...
 */
export function learnableProfiles(corpus: FroamCorpus): FroamPageProfile[] {
...
export function negativeProfiles(corpus: FroamCorpus): FroamPageProfile[] {
  return selectEntries(corpus, { tiers: ['negative'] }).map((entry) => entry.profile)
}

export function corpusStats(corpus: FroamCorpus): FroamCorpusStats {
  const byTier: Record<FroamCorpusTier, number> = { reference: 0, good: 0, baseline: 0, negative: 0 }
  const byViewport: Partial<Record<FroamViewport, number>> = {}
 ...
 */
export function corpusReadiness(corpus: FroamCorpus, input: { minOrigins?: number; minLearnable?: number } = {}): FroamCorpusReadiness {
  ...
  if (learnableOrigins < minOrigins) blockers.push(`${learnableOrigins} learnable origins, need ${minOrigins} — an origin-split hold-out needs sites to split`)
  if (stats.learnable < minLearnable) blockers.push(`${stats.learnable} learnable pages, need ${minLearnable}`)
  if (stats.judgeVersions.length > 1) blockers.push(`mixed judge versions (${stats.judgeVersions.join(', ')}) — scores across versions are not comparable, re-audit before inducing`)

  if (stats.byTier.reference === 0) warnings.push('no reference-tier pages: the corpus has no target to aim at, only an average to regress toward')
  if (stats.byTier.negative === 0) warnings.push('no negative-tier pages: without counter-examples there is no decision boundary')
  if (Object.keys(stats.byViewport).length === 1) warnings.push(`only ${Object.keys(stats.byViewport)[0]} captures: priors will not transfer across viewports`)
  if (stats.multiPageOrigins === 0 && stats.entries > 0) warnings.push('every origin has one page: within-site consistency cannot be measured')

  return { ready: blockers.length === 0, blockers, warnings }
}

/** Re-audit every entry under the current judge, reporting where ingest scores have drifted. */
export function reauditCorpus(corpus: FroamCorpus) {
  const drifted: Array<{ origin: string; routeKey: string; was: number; now: number; delta: number }> = []
