...
test('Pages and Library use the connected project while Reference owns screenshot reconstruction', () => {
  ...
  const leftPanelCss = fs.readFileSync(new URL('../src/editor/styles/left-panel.css', import.meta.url), 'utf8')
  const editingCss = fs.readFileSync(new URL('../src/editor/styles/editing-mode.css', import.meta.url), 'utf8')
  const figmaCss = fs.readFileSync(new URL('../src/editor/styles/figma-layout.css', import.meta.url), 'utf8')
  const triggerCss = fs.readFileSync(new URL('../src/editor/styles/trigger-button.css', import.meta.url), 'utf8')
  ...
  assert.match(layers, /froam-lp__breadcrumbs/)
  assert.match(layers, /Move through nearby layers/)
  assert.match(layers, /Under pointer/)
  assert.match(layers, /Section controls/)
  assert.match(layers, /Add section above/)
  assert.match(layers, /Move section down/)
  assert.match(layers, /Hide in editor/)
  assert.match(layers, /Hide in export/)
  assert.match(layers, /Delete section/)
  ...
  assert.match(editor, /elementsFromPoint/)
  assert.match(editor, /chooseSelectionTarget\(event, stack\)/)
  assert.match(editor, /event\.altKey/)
  assert.match(editor, /syncStructureBoundaryLabel\(target\)/)
  assert.match(editor, /finishSectionMutation/)
  assert.match(editor, /getHostStructurePath/)
  assert.match(editor, /INJECTED_BLOCK_SELECTOR/)
  assert.match(editor, /data-froam-runtime-injected/)
  assert.match(editor, /Duplicated section/)
  assert.match(editor, /Deleted section/)
  ...
  assert.match(leftPanelCss, /\.froam-lp__breadcrumbs/)
  assert.match(leftPanelCss, /\.froam-lp__stack/)
  assert.match(leftPanelCss, /\.froam-lp__section-controls/)
  assert.match(figmaCss, /\.froam-figma-left__view > \.froam-lp/)
