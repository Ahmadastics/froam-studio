...
test('nearby evidence cannot expand mutation scope', () => { const request = contextRequest(); assert.ok(request.context.relationships.some((relation) => relation.from === 'hero')); assert.deepEqual(request.scopeNodeIds, ['cta']) })
...
test('fresh selection evidence avoids project-wide scan and relation enumeration', () => { const project = fixtureProject(); const state = project.checkpoints[project.branches.main.baseCheckpointId].state; state.scans = new Proxy(state.scans, { ownKeys() { throw new Error('scans enumerated') } }); state.relations = new Proxy(state.relations, { ownKeys() { throw new Error('relations enumerated') } }); const request = contextRequest({ project }); assert.equal(request.context.scanRecords[0].id, snapshot.scan.id); assert.equal(request.context.relationships[0].id, snapshot.relationships[0].id) })
...
test('kept Ask Froam styles remain native undo and redo operations', () => { const source = fixtureProject(); const result = createExperiment(source); const branch = result.project.activeBranchId; const comparison = compareMutationBranches(result.project, 'main', branch); const adopted = adoptMutationChanges(result.project, { mutationBranchId: branch, targetBranchId: 'main', eventIds: comparison.eventIds, actorId: 'tester', now: 20, idFactory: ids('undoable-adopted') }); const before = deriveBranchState(source, 'main').legacyStore; const after = deriveBranchState(adopted.project, 'main').legacyStore; const history = createOpLogSession({ actor: 'tester' }); history.seed(before); history.reconcile(after, 'Froam experiment'); assert.equal(history.canUndo(), true); history.undo(); assert.equal(history.store()[viewportKey][snapshot.path].styles.boxShadow, 'none'); assert.equal(history.canRedo(), true); history.redo(); assert.notEqual(history.store()[viewportKey][snapshot.path].styles.boxShadow, 'none') })
