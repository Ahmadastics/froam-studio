...
export function observeResponsiveState(root, registry, policies, width) {
    ...
    for (let index = 0; index < positioned.length; index += 1) {
        const rect = positioned[index].rect;
        for (let x = Math.floor(rect.left / cellSize); x <= Math.floor(Math.max(rect.left, rect.right - 1) / cellSize); x += 1)
            for (let y = Math.floor(rect.top / cellSize); y <= Math.floor(Math.max(rect.top, rect.bottom - 1) / cellSize); y += 1) {
                const key = `${x}:${y}`;
    ...
    for (const candidates of cells.values())
        for (let aIndex = 0; aIndex < candidates.length; aIndex += 1)
            for (let bIndex = aIndex + 1; bIndex < candidates.length; bIndex += 1) {
                const left = Math.min(candidates[aIndex], candidates[bIndex]);
                const right = Math.max(candidates[aIndex], candidates[bIndex]);
                ...
                if (overlapArea(a.rect, b.rect) >= 4 && !a.element.contains(b.element) && !b.element.contains(a.element))
                    collisions.push([a.entry.nodeId, b.entry.nodeId]);
            }
