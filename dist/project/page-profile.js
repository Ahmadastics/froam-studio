...
function classifySection(childRoles, input) {
    ...
    if (input.cardCount >= 3 && input.columns >= 2)
        return { archetype: 'feature-grid', confidence: 0.68 };
    if (input.hasForm)
        return { archetype: 'cta', confidence: 0.6 };
    if (input.cardCount === 2 || input.columns === 2)
        return { archetype: 'split', confidence: 0.5 };
    if ((has('cta') || has('button')) && childRoles.length <= 4)
...
export function buildPageProfile(input) {
    ...
    const sectionNodes = (root?.childIds ?? [])
        .map((id) => byId.get(id))
        .filter((view) => Boolean(view?.visible && view.rect.height >= rootHeight * 0.04 && view.rect.width >= rootWidth * 0.6))
