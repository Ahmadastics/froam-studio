export const FROAM_INTELLIGENCE_SCHEMA_VERSION = 1;
...
const PURPOSES = new Set(['mutate', 'understand', 'reference', 'responsive', 'evaluate']);
const MAX_DEPTH = 12;
...
function validContext(value) {
    ...
    if (!boundedString(value.projectId) || !boundedString(value.activeBranchId))
        return false;
    if (typeof value.routeKey !== 'string' || value.routeKey.length > 2_000)
        return false;
    if (!['desktop', 'tablet', 'mobile'].includes(String(value.viewport)))
        return false;
    if (value.selectedNodeId != null && !boundedString(value.selectedNodeId))
        return false;
    if (value.selectedPath != null && (typeof value.selectedPath !== 'string' || value.selectedPath.length > 4_000))
        return false;
    if (value.selectedDomPath != null && (typeof value.selectedDomPath !== 'string' || value.selectedDomPath.length > 4_000))
        return false;
    ...
    if (value.references != null) {
        ...
        for (const reference of value.references) {
            if (!isRecord(reference) || Object.keys(reference).some((key) => !REFERENCE_KEYS.has(key)) || !boundedString(reference.id))
                return false;
            if (reference.mediaReferenceId != null && !boundedString(reference.mediaReferenceId, 1_000))
                return false;
            for (const dimension of ['viewportWidth', 'viewportHeight'])
                if (reference[dimension] != null && (typeof reference[dimension] !== 'number' || !Number.isFinite(reference[dimension]) || reference[dimension] <= 0 || reference[dimension] > 100_000))
                    return false;
...
export function validateIntelligencePlan(raw, request) {
...
function sanitizeStrings(value, maxItems = 20) {
    return Array.isArray(value) ? value.filter((item) => typeof item === 'string' && Boolean(item.trim())).slice(0, maxItems).map((item) => item.trim().slice(0, MAX_RATIONALE_CHARS)) : undefined;
}
function sanitizeFinding(value, scope) {
    ...
    const evidence = Array.isArray(value.evidence) ? value.evidence.slice(0, 20).flatMap((item) => {
        ...
        if (item.confidence != null && (typeof item.confidence !== 'number' || !Number.isFinite(item.confidence)))
            return [];
        return [{ origin: item.origin, summary: item.summary.trim().slice(0, MAX_RATIONALE_CHARS), source: typeof item.source === 'string' ? item.source.slice(0, 500) : undefined, confidence: typeof item.confidence === 'number' ? Math.max(0, Math.min(1, item.confidence)) : undefined }];
    }) : undefined;
    return {
...
export function validateIntelligenceResponse(raw, request, provider = 'unknown') {
    if (!isRecord(raw) || !boundedJson(raw))
    ...
    if (request.purpose === 'reference')
        return { valid: true, response: { ...common, purpose: 'reference', referenceIds: validIds(raw.referenceIds, 20) ? raw.referenceIds : undefined } };
    ...
    if (request.purpose === 'evaluate')
        return { valid: true, response: { ...common, purpose: 'evaluate', score: typeof raw.score === 'number' && Number.isFinite(raw.score) ? Math.max(0, Math.min(1, raw.score)) : undefined } };
    return { valid: true, response: { ...common, purpose: 'understand' } };
}
export function assertResponseSize(raw) {
    if (utf8Bytes(raw) > FROAM_INTELLIGENCE_MAX_RESPONSE_BYTES)
        throw new Error('Intelligence response exceeds the maximum size');
}
export const REMOTE_INTELLIGENCE_PRIVACY = {
...
export function assertRemoteIntelligenceConsent(consent) {
    if (!consent)
        throw new Error('Remote intelligence requires explicit consent');
}
//# sourceMappingURL=intelligence-transport.js.map