...
export function scoreFingerprint(want, got) {
    ...
    if (want.id)
        weigh(0.5, want.id === got.id ? 1 : 0);
    if (want.text)
        weigh(0.3, textSimilarity(want.text, got.text));
    if (want.className)
        weigh(0.14, jaccard(words(want.className), words(got.className ?? '')));
    if (want.anchorId)
        weigh(0.1, want.anchorId === got.anchorId ? 1 : 0);
    if (want.anchorPath)
        weigh(0.08, want.anchorPath === got.anchorPath ? 1 : 0);
    if (want.ordinal !== undefined)
        weigh(0.05, want.ordinal === got.ordinal ? 1 : 0);
...
export function fingerprintElement(element, root) {
    const anchorId = nearestAnchorId(element, root);
    const anchorRoot = anchorId ? (root.querySelector(`#${CSS.escape(anchorId)}`) ?? root) : root;
    const text = (element.textContent ?? '').trim().replace(/\s+/g, ' ').slice(0, TEXT_SAMPLE);
    const fingerprint = { tag: element.tagName.toLowerCase() };
    if (element.id)
        fingerprint.id = element.id;
    if (text)
        fingerprint.text = text;
    if (element.className && typeof element.className === 'string')
        fingerprint.className = element.className;
    if (anchorId)
        fingerprint.anchorId = anchorId;
    const anchorPath = getElementPath(element, anchorRoot);
    if (anchorPath)
        fingerprint.anchorPath = anchorPath;
    const ordinal = ordinalAmongSiblings(element);
    if (ordinal !== undefined)
        fingerprint.ordinal = ordinal;
    return fingerprint;
}
...
export function resolveAnchor(anchor, root) {
    ...
    let best = null;
    const candidates = root.querySelectorAll(anchor.fingerprint.tag);
