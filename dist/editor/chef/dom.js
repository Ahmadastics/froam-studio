...
export function applyGlobalCSS(css) {
...
export function isSvgInternal(element) {
...
export function readCssUrl(value) {
    const match = value.match(/url\((['"]?)(.*?)\1\)/i);
    return match?.[2] ?? null;
}
export function parseTransformValues(transformStr) {
...
export function readImageUrl(value) {
    const match = value.match(/url\((['"]?)(.*?)\1\)/i);
    return match?.[2] ?? '';
}
export function buildSelection(element, path) {
    try {
