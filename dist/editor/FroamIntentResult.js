...
export default function FroamIntentResult(props) {
    ...
    if (state.phase === 'previewing' && state.session)
        return _jsxs("aside", { ref: surfaceRef, className: "froam-intent-result is-preview", "data-chef-editor-root": "true", role: "dialog", "aria-label": "Froam experiment result", children: [_jsxs("header", { children: [_jsxs("div", { children: [_jsx("span", { children: "Prototype" }), _jsx("strong", { children: state.session.prototypeName })] }), _jsxs("em", { children: [state.session.attempt, "/", state.session.maxAttempts] })] }), _jsxs("p", { children: ["Froam prepared ", state.session.changeCount, " change", state.session.changeCount === 1 ? '' : 's', state.session.targetLabel ? ` for ${state.session.targetLabel}` : '', "."] }), state.session.automaticTarget && _jsx("small", { children: "Froam chose the most relevant page region because nothing was selected." }), state.session.referenceValidation && _jsxs("section", { className: "froam-intent-result__scorecard", "aria-label": "Reference candidate scorecard", children: [_jsx("strong", { children: "Reference match" }), ['structure', 'geometry', 'responsive', 'visual', 'text'].map((kind) => _jsxs("span", { children: [_jsx("b", { children: kind }), _jsx("em", { children: kind === 'visual' && state.session.referenceValidation.scorecard.visual !== undefined ? `${Math.round(state.session.referenceValidation.scorecard.visual * 100)}% measured` : score(state.session.referenceValidation.scorecard[kind]) })] }, kind)), _jsx("strong", { children: "Responsive health" }), _jsx("small", { children: state.session.referenceValidation.health.healthy ? '✓ No overflow, collision, clipping or hidden critical content measured' : `△ ${state.session.referenceValidation.differences.length} measured difference${state.session.referenceValidation.differences.length === 1 ? '' : 's'}` })] }), _jsxs("details", { children: [_jsxs("summary", { children: [_jsx(ChevronDown, { size: 12 }), " What changed"] }), state.session.rationale && _jsx("p", { children: state.session.rationale }), _jsx("ul", { children: state.session.changeSummaries?.map((summary, index) => _jsx("li", { children: summary }, index)) })] }), _jsxs("div", { className: "froam-intent-result__actions", children: [_jsxs("button", { type: "button", className: "is-primary", "data-froam-intent-primary": true, onClick: props.onKeep, children: [_jsx(Check, { size: 13 }), " Keep"] }), _jsxs("button", { type: "button", onClick: props.onRetry, children: [_jsx(RotateCw, { size: 13 }), " Try again"] }), _jsxs("button", { type: "button", onClick: props.onCancel, children: [_jsx(X, { size: 13 }), " Cancel"] })] })] });
