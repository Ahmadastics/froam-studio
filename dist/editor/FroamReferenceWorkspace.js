...
import { FROAM_REFERENCE_ACCEPTED_TYPES, readReferenceConsent, referenceQualityRows, suggestReferenceLabel, validateReferenceDimensions, validateReferenceFile, writeReferenceConsent } from './reference-workspace-model.js';
...
export default function FroamReferenceWorkspace(props) {
    ...
    useEffect(() => () => { for (const url of urlsRef.current)
        URL.revokeObjectURL(url); urlsRef.current.clear(); cancelledRef.current = true; }, []);
        ...
        try {
            ...
            for (let index = 0; index < files.length; index += 1) {
                if (cancelledRef.current)
                    break;
                try {
                    accepted.push(await decodeFile(files[index], index));
                }
                catch (error) {
                    errors.push(`${files[index].name}: ${error instanceof Error ? error.message : 'Could not read screenshot'}`);
                }
            }
            if (cancelledRef.current) {
