...
export default function FroamConnectedCanvas(props) {
    ...
    function commitAnimation(css, inline, interaction) {
    ...
    function saveDraftToArchive(interaction) {
        if (!props.selection?.nodeId)
            return props.onToast('Select a node and create motion before archiving it');
        ...
        setProject((current) => {
            const clock = Math.max(0, ...current.events.map((event) => event.clock)) + 1;
            return appendProjectEvents(current, [
