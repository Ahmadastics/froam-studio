...
export default function FroamSitePlanner({ routeKey, projectName, branchName, requestedTab, selection, archiveItems, assets = [], onRenameProject, onAddAsset, onApplyAsset, onRemoveAsset, onTabChange, onInsertComponent, onInsertBlankFrame, onInsertBlock, onInsertArchived, onBuildPage, onPlanChange, onToast }) {
    ...
    function addMediaUrl() {
        ...
        if (!/^(https?:\/\/|data:image\/)/i.test(url)) {
            onToast('Use an http(s) image URL or upload an image');
            return;
        }
    ...
    return (_jsxs("div", { className: "fsp", "data-chef-editor-root": "true", children: [_jsxs("header", { className: "fsp-context", children: [_jsxs("div", { className: "fsp-context__project", children: [_jsx(Network, { size: 14 }), _jsxs("span", { children: [_jsx("input", { className: "fsp-project-name-input", "aria-label": "Project name", value: projectNameDraft, onChange: (event) => setProjectNameDraft(event.target.value), onBlur: commitProjectName, onKeyDown: (event) => {
                                            if (event.key === 'Enter')
