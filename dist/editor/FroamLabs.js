...
import { Activity, Atom, Beaker, BookOpen, Eye, FlaskConical, Radio, Sparkles, Volume2, X } from 'lucide-react';
...
import { FROAM_PHYSICS_PRESETS, gravityForce, interactionWithGravity, interactionWithPhysics, physicsPreset } from '../project/physics.js';
...
import { animationPresetInteraction, FROAM_ANIMATION_PRESETS } from './FroamAnimationPresets.js';
import { createArchiveItem, minimalArchiveDna } from '../project/archive.js';
...
export default function FroamLabs(props) {
    const [localFlags, setLocalFlags] = useState(() => readFroamLabsFlags(typeof localStorage === 'undefined' ? undefined : localStorage));
    const flags = props.flags ?? localFlags;
    const [active, setActive] = useState(() => { try {
        return localStorage.getItem(LAB_TAB_KEY) ?? 'overview';
    }
    catch {
        return 'overview';
    } });
    ...
    const [gravityStrength, setGravityStrength] = useState(24);
    const [gravityRadius, setGravityRadius] = useState(180);
    const [chaos, setChaos] = useState(null);
    const [synthetic, setSynthetic] = useState(null);
    const [storyboard, setStoryboard] = useState(null);
    const [lastLabInteraction, setLastLabInteraction] = useState(null);
    ...
    useEffect(() => { if (props.requestedLab)
        setActive(props.requestedLab); }, [props.requestedLab]);
    useEffect(() => {
        const flagByLab = { mutate: 'mutate', interactions: 'interactionLibrary', sample: 'uiSampling', physics: 'designPhysics', break: 'chaosTesting', user: 'syntheticUx', sound: 'uiSound', trailer: 'trailerGenerator', reality: 'realityMode' };
        const requiredFlag = flagByLab[active];
        if (requiredFlag && !flags[requiredFlag])
            setActive('overview');
    }, [active, flags]);
    useEffect(() => { try {
        localStorage.setItem(LAB_TAB_KEY, active);
    }
    catch { /* optional preference */ } ; props.onTemporalOwnerChange?.(props.open && active === 'sample' ? 'sampling' : props.open && active === 'trailer' ? 'trailer' : null); }, [active, props.open, props.onTemporalOwnerChange]);
    const updateFlag = (key, enabled) => { const next = { ...flags, [key]: enabled }; setLocalFlags(next); writeFroamLabsFlags(typeof localStorage === 'undefined' ? undefined : localStorage, next); props.onFlagsChange?.(next); };
    ...
    function mutate() { if (!props.selectedNodeId)
        return props.onToast('Select a scanned node before mutating'); setMutating(true); props.onActivityChange?.('mutating'); const delay = matchMedia('(prefers-reduced-motion: reduce)').matches ? 0 : 420; window.setTimeout(() => { try {
    ...
    function previewMotionPreset(motion) { if (!props.selectedElement)
        return props.onToast('Select an element to preview motion'); const config = motion.config; const keyframes = (config.keyframes ?? []).map((keyframe) => ({ ...keyframe.properties, offset: keyframe.offset / 100 })); if (!keyframes.length)
        return; props.selectedElement.animate(keyframes, { duration: matchMedia('(prefers-reduced-motion: reduce)').matches ? 1 : config.duration ?? 600, delay: config.delay, easing: config.easing, iterations: config.iterations === 0 ? 2 : Math.min(config.iterations ?? 1, 2), direction: config.direction, fill: config.fillMode ?? 'both' }); }
    function applyMotionPreset(motion) { if (!props.selectedNodeId)
        return props.onToast('Select a scanned node before applying behavior'); const interaction = animationPresetInteraction(motion, props.selectedNodeId); addEvents([eventFor('interaction.upserted', { interaction }, [props.selectedNodeId], `Applied ${motion.label}`)]); setLastLabInteraction(interaction); previewMotionPreset(motion); props.onToast(`${motion.label} added as a ${interaction.trigger} interaction`); }
    ...
    function previewGravity() { if (!props.selectedElement)
        return props.onToast('Select an element to preview gravity'); const vector = gravityForce({ mode: gravity, strength: gravityStrength, radius: gravityRadius }, { x: 0, y: 0 }, { x: Math.min(96, gravityRadius * .5), y: -28 }); const x = Math.round(vector.x * 2.2); const y = Math.round(vector.y * 2.2); props.selectedElement.animate([{ transform: 'translate3d(0,0,0)' }, { transform: `translate3d(${x}px,${y}px,0)`, offset: .55 }, { transform: 'translate3d(0,0,0)' }], { duration: matchMedia('(prefers-reduced-motion: reduce)').matches ? 1 : 760, easing: 'cubic-bezier(.22,1,.36,1)' }); props.onToast(`${gravity} force preview: ${x >= 0 ? 'right' : 'left'} ${Math.abs(x)}px, ${y >= 0 ? 'down' : 'up'} ${Math.abs(y)}px`); }
    function applyPhysics() { if (!props.selectedNodeId)
        ...
        interaction = interactionWithGravity(interaction, { mode: gravity, strength: gravityStrength, radius: gravityRadius, sourceId: props.selectedNodeId, labOnly: ['stay-near', 'avoid', 'group'].includes(gravity) }); addEvents([eventFor('interaction.upserted', { interaction }, [props.selectedNodeId], 'Design Physics')]); setLastLabInteraction(interaction); previewGravity(); props.onToast(`${preset} + ${gravity} compiled into deterministic runtime behavior`); }
    function saveLabArtifact() { const interaction = lastLabInteraction ?? Object.values(state.interactions).at(-1); if (!props.selectedNodeId || !interaction)
        return props.onToast('Apply or compile a Laboratory behavior before archiving it'); const dna = state.dna[props.selectedNodeId] ?? minimalArchiveDna(props.selectedNodeId, { role: props.selectedElement?.getAttribute('role') ?? props.selectedElement?.tagName.toLowerCase(), tagName: props.selectedElement?.tagName.toLowerCase(), motion: interaction }); const kind = interaction.physics ? 'interaction' : 'motion'; const item = createArchiveItem({ id: `archive:${kind}:${Date.now().toString(36)}`, nodeId: props.selectedNodeId, name: interaction.name, actorId: props.actorId, projectId: props.project.id, branchId: props.project.activeBranchId, dna, kind, interaction, interactionIds: [interaction.id], includes: interaction.physics ? ['motion', 'behavior'] : ['motion'], description: interaction.physics ? 'Laboratory behavior with deterministic physics settings.' : 'Portable motion created in Interface Laboratory.' }); addEvents([eventFor('archive.upserted', { archiveItem: item }, [props.selectedNodeId], `Archived Laboratory ${kind}`)]); props.onToast(`${interaction.name} saved to Archive`); }
    async function breakPage() { if (!flags.chaosTesting)
        return; props.onActivityChange?.('chaos'); try {
        let viewport = { width: window.innerWidth, height: window.innerHeight };
        const report = await runChaosTesting({ scenarios: createDefaultChaosScenarios().slice(0, 4), state, adapter: { apply(event) { if (event.type === 'viewport')
                    viewport = { width: event.width, height: event.height }; }, capture() { const elements = [...document.querySelectorAll('[data-froam-id]')]; return { viewport, nodes: elements.map((element) => { const rect = element.getBoundingClientRect(); const css = getComputedStyle(element); return { nodeId: element.dataset.froamId, rect: { x: rect.x, y: rect.y, width: rect.width, height: rect.height }, visible: css.display !== 'none' && css.visibility !== 'hidden', clipped: element.scrollWidth > element.clientWidth + 2 || element.scrollHeight > element.clientHeight + 2, reachable: !element.matches(':disabled') }; }) }; }, restore() { } } });
        setChaos(report);
        const analysis = chaosReportAnalysis(report);
        addEvents([eventFor('analysis.upserted', { analysis }, analysis.targetIds, 'BREAK THIS PAGE')]);
        props.onToast(`${report.total} realities: ${report.passed} passed, ${report.warnings} warnings, ${report.failed} failed`);
    }
    finally {
        props.onActivityChange?.(null);
    } }
    async function testUser() { const flow = Object.values(state.flows)[0]; if (!flow?.entryNodeId || flow.nodeIds.length < 2)
        return props.onToast('Synthetic UX needs a Product Flow with at least two nodes'); props.onActivityChange?.('synthetic'); try {
        const run = await runSyntheticUx({ id: `goal:${Date.now().toString(36)}`, goal: `Reach ${flow.nodeIds.at(-1)}`, startNodeId: flow.entryNodeId, successNodeIds: [flow.nodeIds.at(-1)] }, { flow, state });
        setSynthetic(run);
        addEvents([eventFor('analysis.upserted', { analysis: syntheticRunAnalysis(run) }, syntheticRunAnalysis(run).targetIds, 'Synthetic UX run')]);
        props.onToast(run.success ? `Synthetic user succeeded in ${run.steps.length} steps` : `Synthetic user failed: ${run.importantFailures[0]}`);
    }
    finally {
        props.onActivityChange?.(null);
    } }
    ...
    const labs = [
        ['overview', 'Overview'],
        ...(flags.mutate ? [['mutate', '☣ MUTATE']] : []),
        ...(flags.interactionLibrary ? [['interactions', 'Interactions']] : []),
        ...(flags.uiSampling ? [['sample', '◉ SAMPLE']] : []),
        ...(flags.designPhysics ? [['physics', 'Physics']] : []),
        ...(flags.chaosTesting ? [['break', 'BREAK']] : []),
        ...(flags.syntheticUx ? [['user', 'Test User']] : []),
        ...(flags.uiSound ? [['sound', 'Sound']] : []),
        ...(flags.trailerGenerator ? [['trailer', 'Trailer']] : []),
        ...(flags.realityMode ? [['reality', 'Reality']] : []),
    ];
    const quickMotions = FROAM_ANIMATION_PRESETS.filter((motion) => !search.trim() || `${motion.label} ${motion.category} ${motion.description}`.toLowerCase().includes(search.toLowerCase())).slice(0, 12);
    const forceVector = gravityForce({ mode: gravity, strength: gravityStrength, radius: gravityRadius }, { x: 0, y: 0 }, { x: Math.min(96, gravityRadius * .5), y: -28 });
    const gravityCopy = { attract: 'Pulls the selected object toward its target.', repel: 'Pushes it away from its target.', follow: 'Tracks a target with a steady pull.', anchor: 'Returns the object to a fixed point.', 'stay-near': 'Keeps related objects within a chosen radius.', avoid: 'Prevents nearby objects from crowding.', group: 'Gathers related objects into a soft cluster.' };
    return _jsxs("aside", { className: `froam-labs ${mutating ? 'is-mutating' : ''}`, "data-chef-editor-root": "true", children: [_jsxs("header", { children: [_jsxs("div", { children: [_jsx(Beaker, { size: 16 }), _jsx("strong", { children: "Experiments" }), _jsx("small", { children: "Optional Froam tools" })] }), _jsx("button", { type: "button", "aria-label": "Close Experiments", onClick: props.onClose, children: _jsx(X, { size: 15 }) })] }), _jsx("nav", { children: labs.map(([id, label]) => _jsx("button", { className: active === id ? 'is-active' : '', onClick: () => setActive(id), children: label }, id)) }), _jsxs("main", { children: [active === 'overview' && _jsxs("section", { className: "froam-labs__flags", children: [_jsx("h3", { children: "Experiment controls" }), _jsx("p", { children: "Enable only the experimental tools you want in this workspace. Enabled tools appear in the Experiment rail." }), Object.keys(flags).map((key) => _jsx(Flag, { flag: key, value: flags[key], update: updateFlag }, key))] }), active === 'mutate' && _jsxs("section", { className: "froam-labs__mutate", children: [_jsx("h3", { children: "\u2623 MUTATE" }), _jsx("p", { children: "Fork first. The source branch is immutable." }), _jsxs("select", { value: level, onChange: (event) => setLevel(event.target.value), children: [_jsx("option", { value: "safe", children: "SAFE" }), _jsx("option", { value: "experimental", children: "EXPERIMENTAL" }), _jsx("option", { value: "unhinged", children: "UNHINGED" })] }), _jsx("div", { className: "froam-labs__chips", children: PROTECTIONS.map((item) => _jsxs("label", { children: [_jsx("input", { type: "checkbox", checked: protect.includes(item), onChange: () => setProtect((current) => current.includes(item) ? current.filter((value) => value !== item) : [...current, item]) }), item] }, item)) }), _jsx("div", { className: "froam-labs__proposal", children: proposal?.summary.map((item, index) => _jsxs("p", { children: [_jsx("b", { children: item.domain }), item.rationale, _jsxs("small", { children: [Math.round(item.confidence * 100), "% \u00B7 ", item.targets, " target"] })] }, index)) }), _jsx("button", { disabled: !flags.mutate || mutating, onClick: mutate, children: mutating ? 'Froam Mutagen active…' : proposal?.requiresConfirmation ? 'MUTATE NOW' : 'Create mutation' }), comparison && _jsxs("div", { className: "froam-labs__compare", children: [_jsxs("span", { children: ["Original", _jsx("br", {}), _jsx("b", { children: comparison.sourceBranchId })] }), _jsx("i", { children: "VS" }), _jsxs("span", { children: ["Mutation", _jsx("br", {}), _jsx("b", { children: comparison.mutationBranchId })] }), _jsxs("small", { children: [comparison.structural, " structural \u00B7 ", comparison.visual, " visual \u00B7 ", comparison.interactions, " interaction"] }), _jsx("button", { onClick: adopt, children: "Adopt compatible changes" })] })] }), active === 'interactions' && _jsxs("section", { children: [_jsxs("h3", { children: [_jsx(BookOpen, { size: 14 }), " Interaction Laboratory"] }), _jsx("p", { children: "Compose and test behavior here. Animator remains the precise keyframe timeline; both save into the same project interaction model." }), _jsxs("p", { children: [Object.keys(state.interactions).length, " interactions are active in this branch."] }), _jsx("input", { placeholder: "Search behavior or motion", value: search, onChange: (event) => setSearch(event.target.value) }), _jsxs("h4", { children: [_jsx(Sparkles, { size: 13 }), " Quick behaviors"] }), _jsx("div", { className: "froam-labs__motion-grid", children: quickMotions.map((motion) => _jsxs("article", { children: [_jsxs("button", { type: "button", className: "froam-labs__motion-main", disabled: !flags.interactionLibrary, onClick: () => applyMotionPreset(motion), children: [_jsx("b", { children: motion.label }), _jsxs("small", { children: [motion.category, " \u00B7 ", motion.config.trigger ?? 'load'] }), _jsx("span", { children: motion.description })] }), _jsx("button", { type: "button", title: `Preview ${motion.label}`, "aria-label": `Preview ${motion.label}`, onClick: () => previewMotionPreset(motion), children: _jsx(Eye, { size: 13 }) })] }, motion.id)) }), _jsxs("div", { className: "froam-labs__actions", children: [_jsx("button", { className: "is-primary", disabled: !lastLabInteraction && !Object.keys(state.interactions).length, onClick: saveLabArtifact, children: "Save latest to Archive" }), _jsx("button", { disabled: !flags.interactionLibrary, onClick: saveFirstInteraction, children: "Save as recipe" }), _jsx("button", { disabled: !flags.interactionLibrary, onClick: applyFirstRecipe, children: "Apply saved recipe" })] }), _jsx("ul", { children: searchInteractionLibrary(library, { query: search }).map((recipe) => _jsxs("li", { children: [_jsx("b", { children: recipe.name }), _jsxs("small", { children: [recipe.category ?? 'Uncategorized', " \u00B7 ", recipe.provenance.kind, " \u00B7 ", previewInteractionRecipe(recipe, { samples: 4 }).length, " preview frames"] }), _jsx("button", { disabled: !flags.interactionLibrary, onClick: () => saveLibrary(deleteInteractionRecipe(library, recipe.id)), children: "Remove" })] }, recipe.id)) })] }), active === 'sample' && _jsxs("section", { children: [_jsxs("h3", { children: [_jsx(Radio, { size: 14 }), " Native Sampling"] }), _jsx("p", { children: "Captures observable events, mutations, styles, layout and timing\u2014not source implementation." }), _jsx("button", { disabled: !flags.uiSampling, className: sampling ? 'is-danger' : '', onClick: toggleSampling, children: sampling ? 'Stop and reconstruct recipe' : '● Record selected region' }), _jsx(Flag, { flag: "externalSampling", value: flags.externalSampling, update: updateFlag }), _jsx("small", { children: "External prototype is separately permissioned under experiments/external-sampler." })] }), active === 'physics' && _jsxs("section", { children: [_jsxs("h3", { children: [_jsx(Atom, { size: 14 }), " Design Physics"] }), _jsx("p", { children: "Physics is deterministic. Preview the force, compile it into an interaction, then keep the result in the shared Archive." }), _jsxs("label", { className: "froam-labs__field", children: [_jsx("span", { children: "Feel" }), _jsx("select", { value: preset, onChange: (event) => setPreset(event.target.value), children: Object.keys(FROAM_PHYSICS_PRESETS).map((name) => _jsx("option", { children: name }, name)) })] }), _jsx("div", { className: "froam-labs__gravity-modes", children: ['attract', 'repel', 'follow', 'anchor', 'stay-near', 'avoid', 'group'].map((mode) => _jsx("button", { type: "button", className: gravity === mode ? 'is-active' : '', onClick: () => setGravity(mode), children: mode }, mode)) }), _jsx("p", { className: "froam-labs__explain", children: gravityCopy[gravity] }), _jsxs("div", { className: "froam-labs__force-preview", "data-mode": gravity, children: [_jsx("i", { className: "is-source", children: "YOU" }), _jsx("span", { style: { '--force-x': `${Math.round(forceVector.x * 2)}px`, '--force-y': `${Math.round(forceVector.y * 2)}px` } }), _jsx("i", { className: "is-target", children: "TARGET" }), _jsxs("small", { children: [gravityStrength, " force \u00B7 ", gravityRadius, "px field"] })] }), _jsxs("label", { className: "froam-labs__range", children: [_jsxs("span", { children: ["Force ", _jsx("b", { children: gravityStrength })] }), _jsx("input", { type: "range", min: "4", max: "60", value: gravityStrength, onChange: (event) => setGravityStrength(Number(event.target.value)) })] }), _jsxs("label", { className: "froam-labs__range", children: [_jsxs("span", { children: ["Field radius ", _jsxs("b", { children: [gravityRadius, "px"] })] }), _jsx("input", { type: "range", min: "80", max: "420", step: "10", value: gravityRadius, onChange: (event) => setGravityRadius(Number(event.target.value)) })] }), _jsx(Flag, { flag: "uiGravity", value: flags.uiGravity, update: updateFlag }), _jsxs("div", { className: "froam-labs__actions", children: [_jsxs("button", { type: "button", onClick: previewGravity, children: [_jsx(Eye, { size: 13 }), " Preview force"] }), _jsx("button", { disabled: !flags.designPhysics, onClick: applyPhysics, children: "Compile interaction" }), _jsx("button", { className: "is-primary", disabled: !lastLabInteraction?.physics, onClick: saveLabArtifact, children: "Save physics to Archive" })] })] }), active === 'break' && _jsxs("section", { children: [_jsxs("h3", { children: [_jsx(FlaskConical, { size: 14 }), " BREAK THIS PAGE"] }), _jsx("p", { children: "Run isolated viewport realities here. Content, assets, locale, network, API, permissions and sessions use the same adapter boundary when a host can safely simulate them." }), _jsx("button", { disabled: !flags.chaosTesting, onClick: () => void breakPage(), children: "Run tested viewport realities" }), chaos && _jsxs("div", { className: "froam-labs__score", children: [_jsxs("b", { children: [chaos.total, " scenarios"] }), _jsxs("span", { children: [chaos.passed, " passed \u00B7 ", chaos.warnings, " warnings \u00B7 ", chaos.failed, " failed"] }), chaos.scenarios.flatMap((item) => item.failures).slice(0, 6).map((failure) => _jsxs("small", { children: [failure.severity, ": ", failure.message] }, failure.id))] })] }), active === 'user' && _jsxs("section", { children: [_jsxs("h3", { children: [_jsx(Activity, { size: 14 }), " Synthetic UX"] }), _jsx("p", { children: "Deterministic Product Flow task runner. Not a substitute for human usability testing." }), _jsx("button", { disabled: !flags.syntheticUx, onClick: () => void testUser(), children: "Run first flow goal" }), synthetic && _jsxs("div", { className: "froam-labs__score", children: [_jsx("b", { children: synthetic.success ? 'Goal completed' : 'Goal failed' }), _jsxs("span", { children: [synthetic.steps.length, " steps \u00B7 ", synthetic.clicks, " clicks \u00B7 ", synthetic.deadEnds, " dead ends"] })] })] }), active === 'sound' && _jsxs("section", { children: [_jsxs("h3", { children: [_jsx(Volume2, { size: 14 }), " UI Sound"] }), _jsx("p", { children: "Project-local optional audio synchronized through FroamInteraction. Browser autoplay rules apply." }), _jsxs("label", { className: "froam-labs__upload", children: ["Import sound", _jsx("input", { type: "file", accept: "audio/*", disabled: !flags.uiSound, onChange: (event) => importSound(event.target.files?.[0]) })] }), _jsxs("span", { children: [Object.values(state.assets).filter((asset) => asset.kind === 'audio').length, " project sounds"] })] }), active === 'trailer' && _jsxs("section", { children: [_jsx("h3", { children: "Trailer Generator" }), _jsx("p", { children: "Editable storyboard from real Product Flow nodes, interactions and mutation branches." }), _jsx("div", { className: "froam-labs__actions", children: [10, 15, 30].map((duration) => _jsxs("button", { disabled: !flags.trailerGenerator, onClick: () => createTrailer(duration), children: [duration, "s"] }, duration)) }), storyboard && _jsx("ol", { children: storyboard.shots.map((shot, index) => _jsxs("li", { children: [_jsxs("b", { children: [(shot.startMs / 1000).toFixed(1), "s"] }), " ", shot.label, _jsxs("span", { className: "froam-labs__actions", children: [_jsx("button", { disabled: index === 0, onClick: () => persistStoryboard(reorderTrailerShot(storyboard, shot.id, index - 1)), children: "\u2191" }), _jsx("button", { disabled: index === storyboard.shots.length - 1, onClick: () => persistStoryboard(reorderTrailerShot(storyboard, shot.id, index + 1)), children: "\u2193" }), _jsx("button", { disabled: storyboard.shots.length === 1, onClick: () => persistStoryboard(removeTrailerShot(storyboard, shot.id)), children: "Remove" })] })] }, shot.id)) })] }), active === 'reality' && _jsxs("section", { children: [_jsxs("h3", { children: ["Reality ", _jsx("small", { children: "Research" })] }), _jsx("p", { children: "Bounded probable-screen detection and quadrilateral rectification are available through the project API. Manual confirmation is mandatory; Froam does not claim camera calibration or scene understanding." }), _jsx("button", { type: "button", disabled: !flags.realityMode, onClick: () => props.onToast('Reality remains a bounded research boundary; import/camera UX is intentionally deferred until evaluated'), children: "Review research boundary" })] })] })] });
}
