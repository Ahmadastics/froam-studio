...
export default function FroamLabs(props) {
    ...
    function applyMotionPreset(motion) { if (!props.selectedNodeId)
        return props.onToast('Select a scanned node before applying behavior'); const interaction = animationPresetInteraction(motion, props.selectedNodeId); addEvents([eventFor('interaction.upserted', { interaction }, [props.selectedNodeId], `Applied ${motion.label}`)]); previewMotionPreset(motion); props.onToast(`${motion.label} added as a ${interaction.trigger} interaction`); }
    ...
    function applyPhysics() { if (!props.selectedNodeId)
        ...
        interaction = interactionWithGravity(interaction, { mode: gravity, strength: gravityStrength, radius: gravityRadius, sourceId: props.selectedNodeId, labOnly: ['stay-near', 'avoid', 'group'].includes(gravity) }); addEvents([eventFor('interaction.upserted', { interaction }, [props.selectedNodeId], 'Design Physics')]); previewGravity(); props.onToast(`${preset} + ${gravity} compiled into deterministic runtime behavior`); }
