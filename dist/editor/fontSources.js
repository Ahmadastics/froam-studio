...
export function fontsByRole(role) {
 ...
 */
export function fontOptionsFor(brandFonts) {
    const options = [];
    ...
    for (const system of SYSTEM_FONTS)
        options.push({ ...system, role: 'system' });
    return options;
}
    ...
    for (const font of value) {
        ...
        for (const face of Array.isArray(font.faces) ? font.faces : []) {
            ...
            if (face.style === 'italic')
                kept.style = 'italic';
            if (BRAND_FONT_FORMATS.has(face.format))
                kept.format = face.format;
            faces.push(kept);
        }
        if (faces.length)
            clean.push({ family, faces });
    }
    return clean;
}
...
function inferFormat(src) {
    ...
    if (dataMatch)
        return FORMAT_BY_EXTENSION[dataMatch[1].toLowerCase().replace(/^x-font-/, '')] ?? null;
...
export function brandFontFaceCss(brandFonts) {
    ...
    for (const font of brandFonts ?? []) {
        ...
        for (const face of font.faces ?? []) {
            if (!isSafeFontSrc(face?.src))
                continue;
            const format = face.format && BRAND_FONT_FORMATS.has(face.format) ? face.format : inferFormat(face.src);
            const source = format
                ? `url("${cssString(face.src)}") format("${format}")`
                : `url("${cssString(face.src)}")`;
            lines.push('@font-face {', `  font-family: "${cssString(family)}";`, `  src: ${source};`, `  font-weight: ${normalizeWeight(face.weight)};`, `  font-style: ${face.style === 'italic' ? 'italic' : 'normal'};`, '  font-display: swap;', '}');
        }
    }
    return lines;
}
/** Family names the design defines itself — these must never be fetched remotely. */
export function brandFontFamilies(brandFonts) {
    const families = new Set();
    for (const font of brandFonts ?? []) {
        const family = typeof font?.family === 'string' ? font.family.trim() : '';
        if (family && (font.faces ?? []).some((face) => isSafeFontSrc(face?.src)))
            families.add(family);
    }
    return [...families];
}
 ...
 */
export function ensureBrandFontStyle(brandFonts) {
    if (typeof document === 'undefined')
        return;
    const css = brandFontFaceCss(brandFonts).join('\n');
    const existing = document.head.querySelector('style[data-froam-brand-fonts]');
    if (!css) {
        existing?.remove();
        return;
    }
