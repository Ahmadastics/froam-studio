...
export function useFroamIntent(props) {
    ...
    const performRequest = useCallback(async (pending) => {
        ...
        try {
            ...
            if (!currentSelection?.nodeId || currentSelection.nodeId !== pending.session.selectedNodeId || !currentElement || !props.root?.contains(currentElement))
                throw new Error('identity_lost');
    ...
    const submit = useCallback(async (input) => {
        ...
        const pending = { session, snapshot, feedback: null };
