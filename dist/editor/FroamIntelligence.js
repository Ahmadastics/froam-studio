...
export default function FroamIntelligence(props) {
    ...
    async function importScreenshot(file) {
        ...
        try {
            ...
            const result = await localScreenshotProvider.reconstruct({ width: bitmap.width, height: bitmap.height, data: pixels.data, mimeType: file.type, name: file.name });
            ...
            commit(events);
            props.onInsertReconstruction(result.regions, bitmap.width, bitmap.height);
            props.onToast(`Reconstructed ${result.regions.length} editable regions locally`);
        }
        catch (error) {
