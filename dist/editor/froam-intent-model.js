...
export function createLocalFroamIntentProposals(snapshot, intent) {
    ...
    if (/bold|stronger text|heavier text/.test(normalized))
        add('typography', 'fontWeight', '700');
    if (/lighter text|less bold/.test(normalized))
    ...
    if (/italic/.test(normalized))
        add('typography', 'fontStyle', 'italic');
    if (/underline/.test(normalized))
    ...
    if (lineHeight)
        add('typography', 'lineHeight', `${lineHeight.value}${lineHeight.unit}`);
    ...
    if (/shadow|depth|premium|polished|prominent|stand out|pop/.test(normalized))
    ...
    if (/display (?:as )?grid|make (?:it|this) (?:a )?grid|grid layout/.test(normalized))
        add('layout', 'display', 'grid');
    if (/display (?:as )?flex|use flex|flex layout/.test(normalized))
    ...
    if (rotate)
        add('motion', 'transform', `rotate(${rotate.value}deg)`);
    if (/smooth|animate|add (?:a )?transition/.test(normalized))
