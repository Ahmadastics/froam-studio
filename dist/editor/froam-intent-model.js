...
export function createLocalFroamIntentProposals(snapshot, intent) {
    ...
    if (requestedColor)
        add('visual', /background|surface|card|section|fill/.test(normalized) ? 'backgroundColor' : 'color', requestedColor);
    ...
    if (/bigger|larger|increase (?:the )?(?:text|font)|make (?:the )?(?:text|font) bigger/.test(normalized))
    ...
    if (/more (?:space|spacing|padding)|breathing room|roomier/.test(normalized))
    ...
    if (/lift|raise|slightly up/.test(normalized))
