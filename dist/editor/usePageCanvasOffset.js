...
export function usePageCanvasOffset(active, getRoot) {
    ...
    useEffect(() => {
        ...
        const measure = () => {
            ...
            if (chrome) {
                ...
                if (shown && rect.top < window.innerHeight / 2)
            ...
            if (root && (top || bottom)) {
                for (const el of Array.from(root.querySelectorAll('*'))) {
                    ...
                    if (cs.position === 'sticky') {
                        if (top && cs.top !== 'auto')
                            rules.push(`${sel}{top:calc(${cs.top} + ${top}px)!important}`);
                        if (bottom && cs.bottom !== 'auto')
                            rules.push(`${sel}{bottom:calc(${cs.bottom} + ${bottom}px)!important}`);
                    }
                    else {
                        const rect = el.getBoundingClientRect();
                        if (top && rect.top < top)
                            rules.push(`${sel}{translate:0 ${top}px!important}`);
                        else if (bottom && rect.bottom > window.innerHeight - bottom)
                            rules.push(`${sel}{translate:0 -${bottom}px!important}`);
                    }
                }
            }
...
//# sourceMappingURL=usePageCanvasOffset.js.map