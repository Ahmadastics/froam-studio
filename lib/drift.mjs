 ...
 */
export function checkRoute({ routeKey, html, viewports, rootScope }) {
  const document = parseHtml(html)
  // A page-scoped design's paths start at <body> (lib/codegen.mjs designRootScope).
  const pageBody = rootScope === 'page' ? queryFirstByTag(document, 'body') : null
  const { element: root, via } = pageBody ? { element: pageBody, via: 'page' } : resolveFroamRoot(document)
  const entries = []
...
export function checkDesign(design, pages) {
  ...
  for (const [routeKey, viewports] of Object.entries(design?.routes ?? {})) {
    ...
    const result = checkRoute({ routeKey, html, viewports, rootScope: design?.rootScope })
