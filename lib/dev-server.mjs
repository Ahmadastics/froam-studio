...
export function createBridgeServer({ port = 4600, froamDir, app = null, serveDir = null, log = () => {} }) {
  ...
  // Intelligence endpoint — optional. If FROAM_AI_API_KEY is not set the
  // endpoint returns a structured "not configured" response so the editor
  // degrades gracefully without breaking anything.
  const aiApiKey = process.env.FROAM_AI_API_KEY
  const aiModel = process.env.FROAM_AI_MODEL
  const aiBaseUrl = process.env.FROAM_AI_BASE_URL
  ...
  async function handleBridgeRoute(req, res, url) {
    ...
    if (url === '/__froam/intelligence/plan' && req.method === 'POST') {
