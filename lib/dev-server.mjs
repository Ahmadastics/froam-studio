...
export function normalizeAppTarget(app) {
  if (!app) return null
  const input = String(app).trim()
  if (/^\[[^\]]+\]\([^)]+\)$/.test(input)) {
    throw new Error('app target looks like a Markdown link — pass only the URL, for example https://example.com')
  }
  const raw = /^\d+$/.test(input) ? `http://localhost:${input}` : input
  const url = new URL(raw.includes('://') ? raw : `http://${raw}`)
  if (/^\/https?:\/\//i.test(url.pathname)) {
    throw new Error(`app target contains two joined URLs — use only ${url.pathname.slice(1)}`)
  }
  if (!['http:', 'https:'].includes(url.protocol)) throw new Error('app target must use http:// or https://')
  return url
}
...
export function createBridgeServer({ port = 4600, froamDir, app = null, serveDir = null, log = () => {} }) {
  ...
  const roomApi = createFroamRoomApi({ file: path.join(froamDir, 'froam.rooms.json'), log })
  ...
  async function handleBridgeRoute(req, res, url) {
    ...
    if (url === '/__froam/repo/status' && req.method === 'GET') {
      const status = await gitStatus(froamDir)
      return send(res, 200, { success: true, exists: fs.existsSync(designPath()), projectExists: fs.existsSync(projectPath()), dir: froamDir, ...status })
    }

    if (url === '/__froam/repo/save' && req.method === 'POST') {
      const { routeKey, viewportMode, store, brandFonts } = await readJsonBody(req)
      ...
      const design = mergeSave(loadDesign(designPath()), { routeKey, viewportMode, store, brandFonts })
