...
export function createBridgeServer({ port = 4600, froamDir, app = null, serveDir = null, log = () => {} }) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      // No cloud backend behind the bridge: answer the editor's cloud-API
      // probes benignly (GET → "nothing published") so it falls back to
      // repo/local drafts without console noise; refuse cloud publishes.
      if (url.startsWith('/api/froam/')) {
        if (req.method === 'GET') return send(res, 200, { success: false })
        return send(res, 501, { success: false, error: 'Cloud publish is not available through the froam bridge — use Save to Repo (Ctrl+Shift+S).' })
      }
