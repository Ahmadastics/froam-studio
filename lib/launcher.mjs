...
export function resolveSafeWorkspaceDir({
  ...
  // `create: false` answers "where would this go?" without touching the disk,
  // so the tester can be shown the folder before it exists.
  const claim = (dir) => (create ? tryCreateDir(dir) : true)
  if (customDir) {
  ...
  if (targetUrl) {
    ...
    if (urlObj && !isLocalhost(urlObj)) {
      ...
      if (claim(targetDir)) return targetDir
  ...
  function projectWorkspaceDir(dir) {
    ...
    return claim(candidate) ? candidate : null
  }
...
/**
 * Walk up from a path inside a project to the project's root.
 *
 * A dev server's own file lives deep inside `node_modules`, so the first
 * `package.json` found going up is the dependency's, not the project's. Skipping
 * anything under `node_modules` is what makes the answer the folder the tester
 * would call theirs.
 *
 * @param {string} startPath a file or directory known to be inside the project
 * @returns {string | null}
 */
export function nearestProjectRoot(startPath) {
  ...
  for (let depth = 0; depth < 40 && dir && dir !== root; depth += 1) {
    const isVendored = dir.split(path.sep).includes('node_modules')
    if (!isVendored && (
...
/**
 * Run a command and return stdout, or '' if anything goes wrong.
 * Never throws: every caller here is a best-effort guess.
 */
...
/**
 * Pull the absolute paths out of a Windows command line.
 *
 * A path cannot simply be read as "up to the next space" — `C:\Users\Ada Lovelace\
 * my site` is one path, and most real Windows home folders contain a space.
 * Unquoted arguments give no way to know where the path ends, so we grow the
 * candidate a word at a time and keep the longest one that exists on disk.
 *
 * @param {string} commandLine
 * @returns {string[]} longest matches first
 */
export function windowsPathsIn(commandLine) {
  if (!commandLine) return []
  const found = []

  // Quoted arguments are unambiguous, spaces and all.
  for (const match of commandLine.matchAll(/"([a-zA-Z]:[\\/][^"]+)"/g)) {
    found.push(match[1])
  }

  for (const match of commandLine.matchAll(/[a-zA-Z]:[\\/]/g)) {
    const tail = commandLine.slice(match.index).replace(/"/g, '')
    const words = tail.split(' ')
    let longestReal = null
    for (let count = 1; count <= Math.min(words.length, 30); count += 1) {
      const attempt = words.slice(0, count).join(' ').trim()
      try {
        if (fs.existsSync(attempt)) longestReal = attempt
      } catch { /* an illegal path is simply not a candidate */ }
    }
    if (longestReal) found.push(longestReal)
    else found.push(words[0])
  }

  // Longest first: the deepest real path is the most specific clue we have.
  return [...new Set(found)].sort((a, b) => b.length - a.length)
}

/**
 * Which process is listening on this local port?
 * @returns {number | null}
 */
function pidForPort(port, timeoutMs) {
  if (process.platform === 'win32') {
    const out = runQuiet('netstat', ['-ano', '-p', 'TCP'], timeoutMs)
    for (const line of out.split(/\r?\n/)) {
      if (!/LISTENING/i.test(line)) continue
      // Match the local-address column only, so a remote :3000 cannot match.
  ...
  const out = runQuiet('lsof', ['-nP', `-iTCP:${port}`, '-sTCP:LISTEN', '-t'], timeoutMs)
  const pid = Number(out.split(/\s+/).filter(Boolean)[0])
  return Number.isInteger(pid) && pid > 0 ? pid : null
}

/**
 * Paths that might be inside the project, best first.
 * @returns {string[]}
 */
function processPathHints(pid, timeoutMs) {
  if (process.platform === 'win32') {
    // Windows exposes no working directory for another process, but a Node dev
    // server is started by absolute path — the project is in its command line.
    const out = runQuiet('powershell', [
      '-NoProfile', '-NonInteractive', '-Command',
      `(Get-CimInstance Win32_Process -Filter "ProcessId=${pid}").CommandLine`,
    ], timeoutMs)
    return windowsPathsIn(out)
  }

  // POSIX reports the real working directory, which beats any guess.
  const cwdOut = runQuiet('lsof', ['-a', '-p', String(pid), '-d', 'cwd', '-Fn'], timeoutMs)
  const cwd = cwdOut.split(/\r?\n/).find((line) => line.startsWith('n'))?.slice(1)
  const command = runQuiet('ps', ['-o', 'command=', '-p', String(pid)], timeoutMs)
  return [cwd, ...[...command.matchAll(/\/[^\s"']+/g)].map((match) => match[0])].filter(Boolean)
}

/**
 * Find the folder on disk that the dev server on this port is serving.
 *
 * Saves the tester from being asked where their own project is. Best-effort by
 * design: a guess that cannot be confirmed returns null so the caller can ask.
 *
 * @param {number} port
 * @param {{ timeoutMs?: number }} options
 * @returns {string | null} an existing project root, or null
 */
export function findProjectDirForPort(port, { timeoutMs = 4000 } = {}) {
  try {
    const pid = pidForPort(port, timeoutMs)
    if (!pid) return null

    for (const hint of processPathHints(pid, timeoutMs)) {
      const root = nearestProjectRoot(hint)
      if (root && isWritableDirectory(root)) return root
    }
  } catch {
    // Any failure here just means we ask the tester instead.
  }
  return null
}
