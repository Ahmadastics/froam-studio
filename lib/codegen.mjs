...
/**
 * An approved change request, applied to a design: only the paths it changed
 * are set (or removed) — never the contributor's whole snapshot, so anything
 * the owner saved since survives. Text in `writtenText` was placed in the
 * source instead (source-writeback.mjs), so it is dropped from the drafts.
 */
export function applyChangeRequest(design, request, { writtenText = [] } = {}) {
